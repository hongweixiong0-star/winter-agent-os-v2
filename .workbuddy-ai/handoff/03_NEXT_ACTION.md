...
1. **`BTN_OPEN_POWER_DETAILS`（`POWER_OVERVIEW` 的「实力详情」）** —— 客户端把 ≡ 图标与名字印成**一个 token**
   （OCR 读作 `三实力详情`，conf 0.901，中心与按钮实际中心差 3 px），精确匹配读不到；两条 phash 注册的
   y 区间（636..685 / 762..832）之间正是按钮画的位置。改法是**读客户端印的名字**，并给那一条记录声明
   `ocr_merged: true`（词在 token 内）。**真机待验**：下一轮 `OPEN_POWER_DETAILS` 在 `POWER_OVERVIEW` 上
   应成功、后帧为 `POWER_DETAILS`；判据看 `episodes.jsonl` 的 `failure_type` 不再出现
   `SEMANTIC_TARGET_NOT_VERIFIED`（历史上它是 8 次/3 小时）。
2. **不可关弹窗** —— 同一目标若本轮解析不到 **2 次**，本轮不再被下一个 goal 重新推导，带真原因结束
   （`blocking_popup` 先于一切 goal 路由，让位只会换回同一步）。**真机判据**：同一个 (屏,控件) 在
   一个运行里最多出现 2 次失败；不再出现"一个运行里同一决定 5 次"（历史：`20260923_140514_506593`）。
3. **根目录 `out*.txt` 已归档** —— 334 → 46，归档 289 个 / 1.98 MB 在
   `learning/archive/root_scratch_20260923/`（`INDEX.json` 记每个文件的来源路径、大小、mtime、判词；
   该目录已加 `.gitignore`）。**没有删除任何文件**，还原就是把条目搬回根目录。

**不要重复**：不要为"看起来像模板给了它"下结论（用**生产函数**在同一帧上跑一遍）；不要把 `POPUP` 当成一屏
（22 种叠层、2717/2717 条读数都带着是哪一种）；不要把"搜索范围内最像"当成"控件一定存在"（要用**当前帧的实际命中**，
并给负向对照）；**改了边界就要验证边界真的动了**（把 `MAX_*` 写在 `run()` 里会让类属性覆盖无效——本轮踩过，
差点把一次无效 A/B 当成"零回归"证据）。
...
CURRENT TASK: every highest-leverage missing skill is DESIGN-BLOCKED — no draft is implementable from the manifest alone (15 NOT_REGISTERED, 5 NO_VERIFIER); see DESIGN-BLOCKED below

WHY: 3 goal(s) BLOCKED, 9 PARTIAL, mean implementation coverage 0.6171. The blocked goals share one small set of never-implemented skills, so one skill purchase can move several goals at once.

CURRENT ROOT CAUSE: SEMANTIC_TARGET_NOT_VERIFIED — 147 in the last 2 day(s), 429 all-time, last seen 2026-09-23T17:43:06.198521+00:00
LAST GOOD COMMIT: e4fd245
CURRENT DIRTY FILES: 598
...
TOP FAILURE IS RANKED BY RECENT FIRST: read `recent` (last 2 day(s), floor 2026-09-21T18:34:17.663078+00:00) before `count` (all-time). A failure type with recent=0 is history, not a current defect.

BLOCKED GOALS: ['ALLIANCE_TIMED_EVENTS', 'USE_FREE_ARENA_ATTEMPTS', 'LABYRINTH_DAILY']
MISSING SKILLS BY LEVERAGE: [('CHECK_ALLIANCE_EVENT', 2), ('CLAIM_EVENT_TIER', 2), ('READ_BEAR_TIMER', 2), ('READ_COUNTER', 2), ('READ_TIMER', 2), ('USE_ACTIVITY_ATTEMPT', 2), ('ALLIANCE_HELP', 1), ('ALLIANCE_TECH_CONTRIBUTE', 1), ('OPEN_ARENA', 1), ('OPEN_BUILDING', 1)]
DESIGN-BLOCKED (not implementable from the draft alone; each needs a live frame of its page first): CHECK_ALLIANCE_EVENT [NOT_REGISTERED] — requires semantic(s) `ALLIANCE_EVENT_ENTRY` that do not exist in dataset/candidate/template_manifest.json; the page has never been observed live, so this needs new vision design first; CLAIM_EVENT_TIER [NOT_REGISTERED] — requires semantic(s) `EVENT_TIER_CLAIMABLE` that do not exist in dataset/candidate/template_manifest.json; the page has never been observed live, so this needs new vision design first; READ_BEAR_TIMER [NOT_REGISTERED] — requires semantic(s) `BEAR_TIMER` that do not exist in dataset/candidate/template_manifest.json; the page has never been observed live, so this needs new vision design first; READ_COUNTER [NO_VERIFIER] — has no design draft at all (absent from winter_agent_v2/skill_factory.PRIORS), so its required semantics and success condition are undefined; READ_TIMER [NO_VERIFIER] — has no design draft at all (absent from winter_agent_v2/skill_factory.PRIORS), so its required semantics and success condition are undefined; USE_ACTIVITY_ATTEMPT [NO_VERIFIER] — has no design draft at all (absent from winter_agent_v2/skill_factory.PRIORS), so its required semantics and success condition are undefined ... and 14 more
