...
- **SEMANTIC_TARGET_NOT_VERIFIED** x423 all-time; recent=156 (last 2d), last seen 2026-09-23T14:00:32.817584+00:00 — TRY_ORDINARY_CONTROL(47), OPEN_HOME(45), SELECT_RESOURCE(45)
...
- **BEAST_DISPATCH_NOT_PROVEN** x51 all-time; recent=50 (last 2d), last seen 2026-09-23T13:44:33.862783+00:00 — DISPATCH_BEAST(51)
- **NO_EXECUTION** x51 all-time; recent=42 (last 2d), last seen 2026-09-23T12:53:43.635831+00:00 — SAFE_STOP(51)
- **EXPLORATION_IDLE_DIALOG_NOT_PROVEN** x39 all-time; recent=20 (last 2d), last seen 2026-09-22T11:21:51.270358+00:00 — EXPLORATION_IDLE_CLAIM(39)
...
- 521 uncommitted file(s): ['M .workbuddy-ai/commander/CODEX_DIRECTIVES.md', ' M .workbuddy-ai/commander/EXECUTION_STATE.json', ' M .workbuddy-ai/commander/LAST_CODEX_REVIEW.md', ' M .workbuddy-ai/commander/WORK_QUEUE.json', ' M .workbuddy/memory/2026-09-21.md']
<!-- /AUTO:open_issues -->
...
| 92 | **金环/教学手指由客户端自己周期性画出，单帧的"有环/无环"不能作为任何判据的证据** | 🟢 **已修（brain + verifier + 接线）** | **主证据（逐像素亲核的一对）**：`17:50:32` 城市视野、面板关闭、**无环**；`17:50:52` 面板读到盾兵行 `已完成`（带绿勾）；`17:51:00` **同一视野**（营房气泡落在同一像素）**画出了环与教学手**。28 秒、面板状态未变而变化的是环 ⇒ 它是客户端按自己节奏画的，不是训练任务的状态，等它变成菜单等于等一个动画 （历史 45/127 步 = 35% 的 `KEEP_TRAINING_PRODUCTIVE` 就耗在这里）。**已实施**：① brain 的 Stage A 不再消耗步数（`MAX_CAMP_MENU_WAITS` 与计数器一并删除），首次看见即给出同名 blocker `camp_entry_is_a_guided_step_not_a_selection`（与原先等两步后的结局相同，省掉两步）；② `verify_camp_menu_reobserved` 不再要求"环还在"，只看页面（上一轮已改）；③ 三个测试文件 + `tools/check_wiring.py` 的三处断言改为钉住"立刻停"。**仍未做**：环的分支位于快捷面板分支之前，而环被读到的比例不低 ⇒ 它**会抢占那条真正能走到训练页的面板行路径**；改它需要先有新的"到达"判据（见 #93），故留待单独一轮。 | ⚠ **更正**：上一轮本行写过的"217 帧 / 62 段交替 / 段长 1–6 帧 / 24 帧落在无关步骤上"**已作废** —— 那套计数用的是读取器的 `page` 与环模板本身，而同一套读取在 `17:51:16` 那一帧上把一个**世界地图**帧（行军面板、`#4298 (644, 451)`、底部 探险/英雄/背包/商店/联盟/城镇）读成了 `HOME` 且环模板匹配成功 ⇒ 计数被假阳性污染，见 #93。上一轮提交信息里"最紧的一对是 17:51:16/17:51:35，同一机位"同样**撤回**（左图是世界地图，且两帧标签与真实状态相反）。结论方向不变，依据换成了上面这一对亲核帧。  ⚠ **第二轮更正**：按**路径**配对（`tools/frame_owner.py`）重做后，比例**成立**且更干净 —— 119 帧城市可见中 71 有环 / 48 无环（60%/40%，与原比例一致），但**交替段数是 22 而不是 62**（62 是时间戳并列导致的假交替）；"24 帧落在无关步骤上"这句**去掉**（那些帧无论属于哪一步都是城市帧）；原 #93（地图帧假阳性）**已撤回**。  ⚠ **第三轮更正（2026-09-23 20:04，受控对比）**：那条注释与 issue 里把标记解释为"客户端按自己的节奏画"。**这一句需要更正**——本题拿到了一对有归属的对照：`20:02` 城市（面板关闭、未点任何东西）**无标记**；`20:03` 城市（本轮起点，仍未点）**无标记**；`20:03` 面板打开（窗内唯一白块是**把手**，270px）；**`20:04` 点下「仓库补给·已完成」行体之后**，城市视野里出现白气泡 **891px @ x 0.4514-0.4958, y 0.3891-0.4437** 并带金环与教学手；`20:08`（四分钟未动、镜头未移动）**同一位置仍在（733px）**。兵营那次（`19:08` 点已完成行体）画出同一形态（519px）。⇒ 标记是客户端**在需要指明某个未处理项时画出的、并持续到被处理**，不是与操作无关的随机脉动。**结论不变、理由更准**：它**仍不能**用来证明"我们的 tap 到达了"（它不是我们那次动作的结果，而是客户端对某个状态的持续指示），`verify_camp_menu_reobserved` 只看页面这一处修改照旧成立。 |
| 93 | ~~读取层在世界地图帧上双假阳性~~ **本条目作废（由我自己的错配造成，不是项目缺陷）** | ⚪ **已撤回** | 我原先记的是"`step_005_before_20260922T175116240696.png` 被读成 `HOME` 且环模板匹配成功"。用 `tools/frame_owner.py` 按**路径**查该帧的归属：它只被一条记录引用 —— `17:51:35 OPEN_HOME`，而那条记录的 `state_before.page` **就是 `MAP`**；环模板在该帧上 `find()` 返回 **None**。两条指控都不成立：读取器没把地图读成 HOME，环也没在地图帧上命中。错因是我按 episode **时间戳**把帧与记录配对（取到的是另一步的文件），与上一轮"最紧的一对"是同一个病根。⇒ 教训与工具见 `tools/frame_owner.py` 与 `.workbuddy-ai/memory/MEMORY.md`。 |
| 94 | **红点驱动的任务触发与处理（操作者指令 2026-09-23，§一-§七）** | 🟠 **§一 已落地；§二/§三/§四/§六 未接；§五 接了一处；§七 未验（需要真机）** | **已做（§一 建立可靠的入口红点读数）**：① 实测五个入口的角标几何（`knowledge/ui/entry_badges.json`）——邮件 9/26 帧有角标、红圆 x 0.943-0.977 y 0.787-0.802（276-302px，中心 0.960/0.794 稳定）；探险 9/26、恒 197px；每日 26/26（计数 278-284px）；联盟 26/26（计数 346-419px）；英雄 26/26 且几何**完全不变** ⇒ 标 `SUSPECT_ARTWORK`，不得当通知读。② `winter_agent_v2/entry_badges.py`：按表读取，三态 PRESENT/ABSENT/UNKNOWN；**没有在表里的入口一律不读**（词典里 5 条 `RED_DOT_*` 已改为 `UNBOUND` 并写明"读了就是数任意红像素"）；入口不在当前页面 → UNKNOWN（不是 ABSENT）；面板关闭或某行未被读到 → UNKNOWN（操作者 §一 明令）。③ `WorldState.red_dots` + `HybridVision.observe` 包装（多返回点无法逐一挂载，包一层才不会漏）。④ `transitions()`：只有真实变化才报；同一红点持续存在不算变化（§二 去重要求）；任一侧 UNKNOWN 不算变化。**§五 已接一处**：`brain.py` 的 MAIL 目标在 HOME 上读到 `BTN_OPEN_MAIL=ABSENT` 时不再进入邮件页（`mail_entry_has_no_badge_this_frame`，非 fatal、让位给其他目标）；PRESENT / UNKNOWN / 无台账 三种情况一律保持原路径。 **未做**：§二 的五类触发条件接入 Goal（除"ABSENT→PRESENT"的变换函数已就绪外，其余四类（首次可靠 PRESENT、页面内出现新子项、受阻事项恢复条件满足、状态变为空闲/免费可领/已完成待领取）都要落到 goal_library/CapabilityGate 的既有机制上）；§三 各入口的分支判断（联盟捐献次数、英雄免费招募、训练/科研以空闲+条件判断而非红点）；§四 结算（领取以实际反馈验收，不得因红点消失宣布到账）；§六 同一入口未变化不重复进入的持久化去重（目前只有 §五 的一处 SAFE_STOP）；§七 七项真机验收全部未做。 | 相关：`entry_badges.py`、`knowledge/ui/entry_badges.json`、`tests/test_entry_badges.py`（16 项）。另注：§七 第 4 项"已完成待领取会继续触发领取"依赖 issue #92/#93 留下的空白——**真正领取已完成批次的控件仍未找到**（按绿勾只关面板、什么也领不到）。 |
| 95 | **红点自主执行 / AI 兜底 / 学习 的真机验收（大指令 §二-§十）** | 🟠 **审计已完成；验收 D 本轮实跑；A/B/C/E/F/G 未做** | **审计结论（本轮，逐条有文件行号）* **本轮实跑（定向 `--goal MAIL`，18:51-18:54，exit=2）**：① `red_dots` 已在正式观测里落地（每步 12 条记录）；三态纪律在线：MAP 帧五条入口全 `UNKNOWN(entry_is_on_HOME_not_MAP)`、面板关闭七行全 `UNKNOWN(quick_panel_is_closed)`；② 邮件入口 18:53:00 读到 **PRESENT 290px**，下一步即 `OPEN_MAIL` → `MAIL_CLAIM_REWARDS` **SUCCESS**（有红点→进入→领取走通，D 项后半）；18:53:54/18:54:27 在 HOME 上读到 **ABSENT 0px**；③ D 项的"跳过"**未被触发**：`[schedule] deferred MAIL_ROUTINE -> DEVELOPMENT_PENDING on OPEN_MAIL_PAGE: a development job owns it (job=1a5835ed)` —— 邮件目标当轮被既有开发作业占着，所以没有任何 mail 决策发生；④ **真机复现并已修**：18:54:27 `OPEN_TASK_FROM_QUICK_PANEL_SHIELD` FAILURE/`PANEL_ROW_TASK_BAR_NOT_PROVEN`——面板打开、盾兵行 `status=IDLE` 但 `control=DONE`（绿勾），TRAIN 分支自己那份行选择器只查了 `arrow_norm` 就把它当"进入"目标点了（通用选择器有 `control==ARROW` 守卫、它没有）⇒ 已补齐两个守卫，并把"只有估算点的行不再被点"一并钉住（50 项测试通过）。  **第三轮补充（本轮）**：① **验收 C 成立且有真机证据**：`18:55:10 OPEN_TASK_FROM_QUICK_PANEL_RESEARCH SUCCESS`，其 after 状态为 `page=HOME, research={"building":"RESEARCH_LAB","menu_open":true}` ⇒ 面板行**打开了科研所自己的任务条**（正是该验证器文档所写的行内结果），紧接 `18:55:28 OPEN_RESEARCH SUCCESS`（其 before 就是 `menu_open=true`）→ `page=RESEARCH`，再 `18:55:56 BACK SUCCESS` 返回。用的是面板行技能本身，不是别的导航路径冒充。② **正式链共同断点（可审计性）已修**：episode 有 `verifier_ok` 布尔值，但 6603 条里只有 **29 条**带验证证据 ⇒ "这步为什么算通过"事后无法审计（本轮我为此反复从帧反推）。已给 `Episode` 加 `verifier_evidence` 并在 `runtime.py` 写入处填上（每个验证器本来就返回该字段）。③ 既有红项判定方法：本轮用 `git stash` A/B 确认 `test_refusal_yields_the_cycle.py` / `test_queue_pump.py` 的 3 项失败**改动前后同样失败**（`3 failed, 44 passed`），与本次改动无关；按 issue #63 的规矩，不引用静态清单。  **第四轮（B 的现场）**：用项目自己的定向打点探针（已按指令给它补上设备租约）做了三次真机实验，得到两条新事实与一次被自己作废的实验——① **已完成行的"行体"与"绿勾"是同一个结局**：面板打开时点 `(225,533)`（盾兵行标签中心，由清单里该行实测 ROI + 该行自身 y 算出），面板关闭，状态变成 `training={"navigation":"INFANTRY_CAMP_HIGHLIGHTED","camp_tap_norm":[0.4243,0.4609]}`——与按绿勾完全一致 ⇒ 二者都是"去看那个营房"，都不是领取控件。② **手所指的目标是一只"营房气泡"**：同一帧上实测其白框约 x 0.44-0.475、y 0.385-0.46（内含一张脸），教学手从 x≈0.465 起遮住右侧，金环在其下方；项目历史四次 tap 打的全是环/地面，**从未打过这只气泡** ⇒ 这是 B 唯一还没试过的下一跳，且需要一个"按帧定位气泡"的读数（朴素的"近白且不偏蓝"掩码不行：只得到 821 个散点、每行约 18 个，不能与雪地区分）。③ **作废的那次**：我把把手点 `(13,550)` 从测试桩里照搬来当"开面板"的第一步，结果点开的是**军师卡牌页**（「药到病除」），于是那三跳从头一步就在量错屏；上一轮它"能用"是巧合。⇒ 把手必须**按帧定位**（`find_quick_panel_handle`），这正是指令 §四"不得把别的页面里同名按钮的固定坐标照搬"那条。该教训已写进探针自己的文档串。④ 真机现状：客户端停在那个卡牌页（读取器判 UNKNOWN），按设计由 AUTO 下一轮的 UNKNOWN 处理把它挪开；本轮未再继续点击以免制造更多无据状态。  **第四轮补充（本轮）**：① **修掉正式链上的一处共同断点**：营地工具条有**两种布局**，而 y 带与门模板都只在空闲布局上量过 ⇒ 忙的营地（`详情/立即完成/加速/训练`，无 `升级`，词整体上移 0.030 到 y 0.683-0.685，**低于 y 带下沿 0.69**）读取器**根本不会被调用**，于是「客户端明明画着工具条」的帧被记成没有工具条 —— 这正是 `16:42:30`、`18:54:27` 两次 FAILURE / `PANEL_ROW_TASK_BAR_NOT_PROVEN` 的成因。已修：y 带取两布局并集 `(0.66, 0.78)`；另为 `TARGET_CAMP_ACTION_BAR` 增第二条门记录（**详情**按钮的图标，两布局 d≤2），`升级` 那条保留；`--check` 现覆盖两布局，结果 **0 miss / 0 假阳性**。实测 `16:42:30` 由 `{}` 变为 `{"menu_open": true, "camp": "SHIELD_CAMP", "train_tap_norm": [0.7312, 0.6844]}`。② **负面结果（防下一轮重走）**：营地上方那只白气泡是**通用建筑提示框**，屏幕位置固定、**内容随对象变**；40 帧实测「x 0.42-0.52 / y 0.35-0.48 有白块」在营地可见时漏 8 次、营地不可见时误报 13 次，三张裁图里它分别装着**士兵头像**与**金色雕像** ⇒ 与 #92 的金环同类问题，**不能**用「有气泡」当营地判据，要落在气泡**内部图标**上。 ③ 顺带修：两个清单写入工具用 `indent=2` 而磁盘上是 `indent=1`，加一条记录会重排全部 408 条（7499 增 / 7359 删），已改 `indent=1`，本次清单 diff 23 行。   **第五轮补充（本轮）**：① **验收 C/F 的真机证据（03:32-03:35，正式 AUTO，就在上一轮修复之后）**：`19:32:34 OPEN_TASK_FROM_QUICK_PANEL_SHIELD SUCCESS`，面板开着、盾兵行 `status=IDLE control=ARROW basis=ROW_BUTTON_SCAN`，after 为 `{"menu_open": true, "camp": "SHIELD_CAMP", "camp_label": "盾兵营", "train_tap_norm": [0.6736, 0.7152], "source": "ACTION_BAR"}`，验证证据 `{"task_bar_open_after": true, "camp_named_after": "SHIELD_CAMP"}`；`19:33:04 OPEN_INFANTRY_TRAINING SUCCESS`：`HOME -> TRAINING`，其 before 正是那个 `menu_open=true`，证据 `{"menu_before": true, "training_after": true}`；`19:33:34 TRAIN_TROOPS SUCCESS`：`status AVAILABLE -> IN_PROGRESS`、`timer 03:28:51`、`batch_count 282`，证据 `{"queue": {"status_ok": true, "timer_ok": true, "queue_busy": true}}`。⇒ **面板行 → 营地径向菜单 → 训练页 → 真的开始一批训练**，全链在设备上走通并各自通过验证器；这条链正是 `16:42:30`/`18:54:27` 死掉的那条。（注意：这次 `train_tap_norm.y=0.7152` 属**空闲布局**，所以它**没有**顺带证明忙布局那条修复。） ② **B 被实测收窄**：训练页在两种状态下都**没有任何领取控件**——队列进行中时是 `立即完成 ⊖337` / `加速`，队列空闲（选数量）时是 `− / ‖ / +` 滑条 + `立即完成 ⊖2,786` + `训练 ⊖03:28:53`；122 次训练页读数分布 `IN_PROGRESS 91 / AVAILABLE 18 / UNKNOWN 12`，兵种数量（2,673/800/4,992）**直接显示**。⇒ 兵营的 `已完成` 极可能是**「该批已训练完毕（部队自动入库）」的通知**，而不是待领取奖励；可领取的入口在 `我的奖励`/邮件/联盟/活动。**未证**：存档里没有"刚完成那一刻"的训练页帧。③ 另发现（登记 #98）：`QUICK_PANEL_ROW_*` 的 `arrow_norm.x` 被扫描带外的城市蓝色拉偏，tap 会落在按钮外；这是当前生产里仍会复发的失败。   **第六轮补充（本轮）**：修掉 **#98** —— 面板行箭头的落点原先被扫描带外的城市蓝色拉偏（详见 #98：按钮在 x 384-425，读取器报 0.5979-0.6667，三次用它的 tap 全失败、落在按钮内的那次成功）。改用**按钮内的白色雪佛龙**定位，40 次定位实测 37 次命中且**中心 spread 0.0000**，未命中的 3 次恰是 已完成 行（客户端画的是绿勾）。8 帧面板复读：`arrow_x` 全回 0.5618、徽标状态未变。   **第七轮补充（真机，20:02-20:08，租约内）**：① 面板行落点**已真机验证**（见 #98）；② 忙布局读数**已真机验证**（见 #97）；③ **点「已完成」行的行体**：`MY_REWARDS`（仓库补给）行体（标签 `仓库补给` 在 x 183-267 y 809-835 → tap (225,822)）→ **面板关闭、页面仍 HOME**，客户端随即画出**手 + 白气泡（内含金色物资箱）+ 地上的金环**；与兵营那次同形态 ⇒ "点已完成行"= 客户端把**所指的东西**标出来，**不是**领取；④ 那只气泡的定位器**经五帧实测后判定不成立**（见 `tools/live_panel_arrow_check.py` 文档头：点击前 0px、面板打开时最大白块是把手 270px、真实气泡 519-891px），因此 `--measure-callout` **只测量不点**，并写明下一次要么补帧、要么改用气泡**内部图标**作依据；⑤ 新缺陷 **#99**（面板行的 `label` 可以取自面板之外的文字）。 **第八轮补充（气泡语料，2026-09-23 20:12-20:30）**：把**全部 1413 张"帧自己的记录说 page=HOME 且面板关闭"**的帧逐帧量了气泡窗口（x 0.28-0.68 / y 0.24-0.58）的**最大近白连通块**，按 box 宽高聚类得 81 种形状。**推翻上一轮的计划**：\"约 400px 的尺寸间隙"不成立——非气泡物体最高到 **1069px**（建筑名称条 65x21）、**921px**（一条宽短白条），而**被教学手遮住的气泡只有 395-520px**，尺寸两个方向都会错。 **形状才是判据**：气泡是**高 58-76px**的白块（全幅 59-70 宽；被手遮住只剩 13-34 宽但仍 58-60 高），而其余白块都矮——建筑等级字形 20-23、面板把手 29、名称条 21-23。取 **≥55px 高**后：语料里 588 帧命中、191 帧不命中，抽查两侧 60 帧得**真阳性 30/40**（被拒的 10 帧全是字形/名称条）、**假阳性 0/20**。 已实现的定位器见 `tools/live_panel_arrow_check.py` 的 `CALLOUT_MIN_HEIGHT_PX` 与 `_largest_bright_blob`，并新增 `--tap-callout`（tap 该气泡中心）。**气泡的身份也看清了**：白色圆角方块 + 底部小尾巴，里面装**被高亮建筑的图标**（显微镜=科研所、钢盔士兵=兵营、金色物资箱=仓库）⇒ 它是客户端"去这里看"的指示，不是奖励弹窗。 **仍未知**：点它会发生什么（本轮真机尝试没走到那一步，见 #101）；因此\"点气泡能领到奖励"**仍未证明**。 |
① 快捷面板不是 TRAIN 私有：`brain.PANEL_ROWS_FOR_ROUTE` 已把 TRAIN→CAMP、RESEARCH→RESEARCH、ALLIANCE→ALLIANCE_DONATION、DAILY→MY_REWARDS 四条路由接上；`brain.py:1106` 的**通用**分支在`world.page is HOME and self._panel_rows_for_this_goal()` 时生效，面板未打开则请求"一个普通控件"，由运行时词典层解析成**当场点把手**（注释注明真机验过 `QUICK_PANEL_OPENED`）。⇒ 指令担心的"识别器/解析器都有但决策分支不请求它"**不成立**；唯一缺口是**英雄招募没有自己的路由**（代码里写明"stated here rather than invented"，未擅自补）。
② AI 兜底通道是**真实且自动**的：`tools/unknown_ai_worker.py --state` 显示 gateway=up、pending=2、answered=1、attempts={b6546e80:1, 23759997:2, 78694f1b:1}；`learning/unknown_dispatch.jsonl` 有 10 条 submitted/reconciled 成对事件；`tools/unknown_chain_check.py` 把 3 条真实 UNKNOWN 请求逐段转换：其中 `unknown__control__b6546e80`（UNKNOWN::燃霜矿区，goal=DAILY）已有答案，转成 `ORDINARY_CONTROL[奖励入口]`、锚定文字"奖励"→ **(635, 372) px**、risk=allowed、expected="奖励列表或奖励预览面板打开"。另外 2 条仍在等答案（其中 23759997 已撞 2 次上限）。
③ **但**：`learning/episodes.jsonl` 里**没有任何** advice/unknown 字段 ⇒ "答案被某一步消费并经 MAA 执行"**无法从日志证明**。
**已做**：§一 入口红点读数（上一轮，issue #94）；§五 邮件入口 ABSENT 跳过（上一轮接线，本轮实跑验收 D）；§九 快捷面板共用性审计（见上）。**未做**：A 无完整 Skill 的红点事项全程走通（进页面→理解→执行→验证）；B "已完成、待领取"真正领到免费奖励——**仍卡在"真正领取已完成批次的控件"未找到**（#92 实测按绿勾只关面板）；C 至少一项城内任务**通过对应面板行**进入并完成后续操作（需真机，且需按 §九 要求在报告里区分"通过面板行进入"与"通过旧电力页等其他路径进入"）；E 由正式 AUTO 校验 AI 建议并经 MAA 执行（通道在，但**消费与执行的证据缺失**，不能算完成）；F 同类事项复用已有 L1/Skill；G 单事项受阻时全局不进入无意义等待（部分可由本轮日志观察）。**§四/§五/§七 的机制**（无 Skill 时的自主候选动作、五态结算词汇 IN_PROGRESS/TASK_COMPLETED/REWARD_CLAIMABLE/REWARD_CLAIMED/RESULT_UNKNOWN、成功单步转 L1）**未实现**。 |
| 96 | **红点作为调度优先级信号与抢占规则（第二份文档 §一-§六）** | 🟠 **§一/§二② 已做并已回放验证（见 #102）；§二③④⑤⑥ 仍未做** | 现有 Scheduler 只看 Goal 的 READY/IN_PROGRESS/COMPLETE/BLOCKED/UNKNOWN 与 `goal_utility.rank`，**没有任何红点权重**：红点目前只被读取（`WorldState.red_dots`、面板 `rows[].badge`），不进入打分。要落地的六条：① 新红点及时唤醒对应 Goal；② 按"即将过期/可立即领取/可立即完成/需进一步观察"排序，红点本身不自动最高优先；③ 保护已到最后一步的 Goal（普通红点不得反复打断）；④ 未知红点有界探索后让位并记恢复条件；⑤ 任务完成→奖励待领取保留较高优先级直到实际领取；⑥ 真机验收：不因长期不消的红点饿死训练/科研，受阻后及时继续其他 Goal，并报告"新红点从发现到首次有效操作的耗时"、是否发生重复进入/无意义等待/普通任务饥饿。**注意**：`goal_utility.rank` 与 `Deferral.until` 是既有机制，改这里要同时给出"不抢占最后一步"的判据，否则会退回操作者点名的老毛病。 |
| 97 | **被按下的"已完成"行的结局读不出来（成因未定）** | 🟠 **已定性 + 因果已撤回** | 事实部分：`18:54:27`（面板盾兵行 `status=IDLE / control=DONE` 被按）之后，帧 `dataset/raw/live_runtime/live_runtime_step_008_after_refresh_2_20260922T185419887295.png` 上 `training={}`，路由无点可打。**因果撤回**：我原先写"这次 tap 画出了金环光晕+气泡"，不成立——① 用项目自己的模板判定，气泡下方那座建筑是 **`BUILDING_RESEARCH_CENTER` 0.654**（兵营只有 0.257），是**科研所**不是兵营；② 同一标记在同一技能的**无关步骤**帧上也出现（`17:55:01` 得 0.655）。⇒ 与 #92 同类：**客户端自己画的标记不能归因于我们的动作**。另据 `19:31:04` 的面板读数，`盾兵` 行已回到 **`空闲中`**（不再是 `已完成`）⇒ 该状态是**瞬时通知**。 | **仍未定**：按下去到底发生了什么（面板关闭 + `training={}`）。下一步需要在真机上做一次**受控**实验：同一步取前后帧、并同时保存面板读数与页面读数。  **真机补充（2026-09-23 20:04）**：同一轮里 `SHIELD_CAMP` 已是 `IN_PROGRESS`（19:33 起的那一批），打开营地面板读到的 `train_tap_norm=[0.7312,0.6844]` —— **正是离线量出的忙布局高度**（离线：忙 `详情 (0.269,0.683)` / `训练 (0.732,0.685)`；空闲 `训练 (0.672,0.714)`）。⇒ 上一轮"忙布局修复尚未在真机观察到"这一条**已补上**。 另：同一帧上 `TARGET_INFANTRY_CAMP_HIGHLIGHTED` 匹配为 **None**，而画面上确实画着金环（尺寸小于该模板的 `≥0.12×0.05` 要求），所以 `training={}` 的原因仍成立。 |
| 98 | **快捷面板行箭头的落点被扫描带外的蓝色拉偏** | 🟢 **已修（ocr + 测试）** | **成因（逐像素复现）**：扫描带 `(0.45, 0.80)` 右沿 = x 576，而面板右缘约 476-484 ⇒ 带内含 155px 的蓝色城市；蓝的**范围取 min..max**、且**取最宽的一行为准**，于是**优先**选中被污染的行。`19:31:04` 帧四行读作 384-477 / 384-498 / 386-574 / 401-494（中心 0.5979-0.6667，最后一个已在面板外），而按钮本体青色 `(76,198,244)` 在 **x 384-425**。**账本证据**：用 `arrow_norm` 的三次 tap 全失败（`19:32:04 [448,804]` 面板没关没动 + `PANEL_ROW_RESEARCH_BAR_NOT_PROVEN`、`17:16:14 [480,804]`、`17:54:12 [448,804]`），而落在按钮内的 `18:55:10 [404,804]` 与落在行标签的 `16:33:10/16:41:59 [225,546]` 都成功。 **修法**：改用**按钮内的白色雪佛龙**定位按钮（`QUICK_PANEL_CHEVRON_MIN=245`，因面板压暗城市、最亮城市像素只有 200-227），再把蓝色只在该邻域内读（`QUICK_PANEL_BUTTON_PAD_PX=12`）；徽标窗口从"按钮左三分之一"改为**右三分之一**（红点实测在 412-420，即按钮右端；旧注释与旧窗口都是照着被污染成 0.539-0.644 的框写的，已一并更正）。 **定位器实测**（10 帧面板 × 4 行 = 40 次）：**37 次命中，且每次都是 x 396-416（宽 20px，中心 0.5639，spread 0.0000）**；3 次未命中**恰是 已完成 行**（19:03:02/19:03:12/19:10:52 的盾兵行——那里画的是绿勾不是箭头）⇒ "没有雪佛龙"就是"客户端没画进入箭头"的正确答案。 **验证**（8 帧面板重读）：`arrow_x` **全部回到 0.5618**（按钮中心，原 0.5979-0.666），徽标状态**一条未变**；顺带的效果是 已完成 行现在落到带标签的估算回退（进入路径本就拒绝它）⇒ "已完成行不是进入目标"由**构造**保证，而不是靠手写条件。 **未在真机观察**：修好后尚未有真机轮再走这条面板行路径。 | 影响面：`QUICK_PANEL_ROW_*` 的进入类落点。原 `QUICK_PANEL_ARROW_BAND_X_NORM` 与死变量 `gap_px` 已删除。  **真机验证（2026-09-23 20:03，`tools/live_panel_arrow_check.py`，持租约）**：把手按帧定位为 `COLLAPSED @ (0.0181,0.4301)` → 点它开面板成功；面板四行读数 `SHIELD_CAMP ARROW [0.5618,0.4262]`、`MARKSMAN ARROW [0.5618,0.5402]`、`RESEARCH ARROW [0.5618,0.6285]`，**每行 `arrow_box_norm.w_norm` 都是 0.0569**（一个控件一个宽度）；滚动后的行同样 `[0.5618, 0.3207/0.4086/0.4965]`（x 完全相同）；点 `SHIELD_CAMP` 自己那个点 **(404,546)** → 营地面板打开：`training={"menu_open":true,"camp":"SHIELD_CAMP","camp_label":"盾兵营","train_tap_norm":[0.7312,0.6844]}`。而 `LANCER_CAMP`（`已完成`）读作 `control=DONE / basis=PANEL_RELATIVE_ESTIMATE / box=None` —— **不给出落点**，与构造性守卫一致。 对照账本：修复前用 `arrow_norm` 的三次 tap（`19:32:04 [448,804]`、`17:16:14 [480,804]`、`17:54:12 [448,804]`）都失败，修好后 `[404,804]` 成功。 |
| 99 | **快捷面板行的 `label`/`status` 可以取自面板之外的文字；同一行因此被读成"非空闲"** | 🟢 **已修（ocr + 测试）** | **机制（已定位）**：`rows_between` 取"表头之后、下一表头之前"的 token 时**只按 y 排序、不判位置**，而面板画在城市之上、城市右侧活动栏的文字与面板行同高 ⇒ `梦境寻忆`（y 0.2977，**x 0.769**）恰好落在 `X科技研究` 表头（y 0.2898）与真正的行（y 0.3211）之间，于是成了该节的"第一行"：`key=RESEARCH, label=梦境寻忆, status=IN_PROGRESS, word=None, control=NONE`，而真正的 `科技研究 / 空闲中 / 蓝箭头` 那一行**完全没有被报出**。 **修法**：新增 `drawn_by_the_panel(token)`（用既有的 `QUICK_PANEL_COLUMN_MAX_X_NORM`＝0.55 判左缘，该常量本就是"面板自己画的 token"的表达，其注释记录实测带 0.10-0.34），三处取 token 组装行的地方（`rows_between`、`state_below`、面板级兵营行）都用它；帧宽读不到时不过滤（与几何段的既有取舍一致）。 **验证**：同一帧改后读作 `科技研究 / 空闲中 / control=ARROW / badge=PRESENT / y=0.3211`，**与真机 20:03 同槽位读数（0.3207）一致**；该滚动组 5 帧一致，其余三行不变。 **端到端（同一帧注入旧读数对比）**：旧读数 → `OPEN_POWER_OVERVIEW`（`research_goal_requires_power_route`，绕远路）；真实读数 → `OPEN_TASK_FROM_QUICK_PANEL_RESEARCH`（`quick_panel_row_research_enters_its_own_task_page`，直接从面板行进）。 | 测试 `TheRowMustBeThePanelsOwnTests`（真机帧 + 注入诱饵 token 两向）；证据帧 `dataset/truth_audit/quick_panel_row_20260923/panel_scrolled_sections_with_the_city_event_rail__live_20260922T2202.png`。 |
| 100 | **面板上"以自身词表状态"的行（`可捐献25/25`、`免费招募`）被通用行选择器按 `status == IDLE` 拒掉；英雄招募还根本没有路由域** | 🔴 **未修（有实测）** | 实测（同一帧，`panel open`）：`ALLIANCE_DONATION` 读 `可捐献25/25`、`control=ARROW`、`badge=PRESENT`；`HERO_RECRUIT` 读 `免费招募`、`control=ARROW`、`badge=PRESENT`。而 `RuleBrain._actionable_panel_row` 要求 `status == "IDLE"`，而这两行因自身词不是 "空闲中" 被读成 `IN_PROGRESS` ⇒ **永远不会被通用选择器提供**。逐目标复算：`goal ALLIANCE -> OPEN_ALLIANCE reason=alliance_goal`（走联盟页，不走行）；`goal HERO_RECRUIT -> OPEN_MAP reason=first_ready_p0_skill`（连行都不看）。 **英雄招募的另一半**：`ROUTE_DOMAINS` 里**没有任何招募类路由域**（HOME/GATHER_RESOURCE/BEAST_HUNT/INTEL/MAIL/EXPLORATION/DAILY/ALLIANCE/RESEARCH/TRAIN），所以 `--goal` 连"请求这条路由"都做不到；技能 `OPEN_TASK_FROM_QUICK_PANEL_HERO_RECRUIT` 已注册（`brain.py:31`）但无处发出；且 `goal_library.py:228` 记着 `DAILY_HERO_RECRUIT` **没有绑定 live-loop 验证器** ⇒ 接上之前先要有验收手段。 **修法方向**（操作者 §三：「明确显示免费招募时可以触发」「联盟捐献结合可捐献次数与实际资源条件」）：把"本行自身的状态词表示有一次可用动作"（`可捐献X/Y`、`免费…`）作为**进入**该行页面的理由，而不是要求 `空闲中`——只是进入页面（导航，低风险），捐献/招募的资源判断仍留在各自路由；`state_below` 早已把这两个词识别为状态，读数已具备，缺的是决策层这条规则。 **未做**：本轮只登记，不接（接了要同时给出验证器与真机验收窗口）。 | 影响面：`PANEL_ROWS_FOR_ROUTE`、`_actionable_panel_row`、`ROUTE_DOMAINS`。 |
| 101 | **世界地图的资源搜索面板压住城镇门 ⇒ `OPEN_HOME` 反复点空：323 次里失败 44 次（13.6%）** | 🟠 **主机制已修（brain 四处补守卫）；另两种成因未修** | 数字：`OPEN_HOME` 共 323 步，**44 次 FAILURE**，**全部 `page_before=MAP`**；其中 37 次 `SEMANTIC_TARGET_NOT_VERIFIED`（所需控件**不在帧上**）、7 次 `OPEN_HOME_NOT_PROVEN`。并成串发生：`15:55-15:58` 四分钟里 8 次、`22:37` 2 次、`00:00-00:01` 2 次；**最近一次 `20:26:44`，就发生在当晚正在跑的那一轮里**（该轮 04:19:41 启动）。 受控对比（同一 ROI `x 0.850-1.000, y 0.910-1.000` = 登记的门位）：**失败帧** `..._step_012_before_20260922T202628471561.png` → 读作 `Page.MAP`、`BTN_RESOURCE_SEARCH_SUBMIT` **在场 d=0**、`BTN_OPEN_HOME` **absent**；**成功帧** `..._step_016_before_20260922T193003809391.png` → `Page.MAP`、搜索面板**不在场**、门 **present d=2**。 ⇒ 资源搜索面板（底部，含 巨兽/生肉/木材/煤矿/铁锭 + 搜索）**正好盖住门所在的右下角**，而页面仍读作 `MAP`，于是回主城这一步每次都在点一个不在那儿的控件。 **同一次会话里我的探针撞到同一面墙**：`tools/live_panel_arrow_check.py` 在 `20:25:56` 的该视图上`BTN_OPEN_HOME` 匹配不到 → 按设计**中止不猜**（`ABORT: on MAP with no registered way home`）⇒ 这不是探针的毛病，是同一处空白。 **未测出**：客户端在这个视图里**怎么关掉面板**。该帧上放大镜 `BTN_OPEN_RESOURCE_SEARCH` **不在场**（"再点一次放大镜关闭"因此不可用），登记里也没有该面板的关闭控件；`BTN_OPEN_MAIL` 在该帧上是 **d=24** 的假阳性（面板内容与它松散相似，threshold 6 会拒掉）。 **修法方向**（两条，先量再改）：① 量出该面板的关闭方式，然后在 `OPEN_HOME` 之前先关它；② 或者在决策层让"面板在场时的 OPEN_HOME"不发出（拒绝一步，好过点空一步）——后者不依赖新控件，但要先确认运行时确实**只在**该视图下拿不到门。 | 影响面：`OPEN_HOME` 技能与其验证器、任何需要从地图回主城的 Goal。证据帧：`learning/_camp_menu/door_roi_failed_vs_ok.png`、`learning/_search_panel_controls.txt`。  **判别器与三条成因（本轮量完）**：取两侧各 30 帧（都是 `page_before=MAP` 的 OPEN_HOME 步）：**成功侧**——搜索面板在场 **0/30**、门在场 **30/30**；**失败侧**——面板在场且门不在场 **22/30**、门在场但步仍失败 **8/30**。⇒ 主机制确认为"搜索面板盖住门"（约占失败 73%）。剩下 8 次又分两种，**都看图核过**（`learning/_camp_menu/door_roi_two_failing_cases.png`：红框里就是发光的「城镇」门，`BTN_OPEN_HOME` **d=2**）——① 4 次 `SEMANTIC_TARGET_NOT_VERIFIED`：**控件明明在帧上、模板 d=2，运行时却解析不出落点**，属**解析器层**的不一致（不是模板假阳性）；② 4 次 `OPEN_HOME_NOT_PROVEN`，其中 3 次的 after 页是 **POPUP** ⇒ tap 发出去了、客户端答以一个弹窗（疑似退出确认类），没有回城。两种都需要各自单独看，**本轮不改代码**（没有实测的替代动作，且无法真机验证）。 证据：`learning/_search_panel_discriminator.txt`、`learning/_open_home_other_cause.txt`、`learning/_open_home_popup_case.txt`、`learning/_camp_menu/door_roi_failed_vs_ok.png`。 **已修（本轮）**：七个 `OPEN_HOME` 发出点里只有三处（MAIL/RESEARCH/TRAIN）在发出前先关搜索面板（`if world.resource_search_open: BACK ... resource_search_closed`），另**四处**（EXPLORATION / DAILY / ALLIANCE / 兜底 `HOME`）没有 ⇒ 已补齐同样的三行，理由名各自独立（`close_resource_search_for_exploration_goal` / `..._daily_goal` / `..._alliance_goal` / `close_resource_search_to_go_home`），satisfier 仍是既有的 `resource_search_closed`、验证器与 `BACK` 都是现成的。 **触发证据就是运行时自己的日志**：`20:26:44 OPEN_HOME` FAILURE（该帧搜索面板在场、门 absent）→ `20:27:15 BACK` SUCCESS 且 `verifier_evidence={"search_closed": true, ...}` → `20:28:16 OPEN_HOME` SUCCESS（该帧面板不在、门 d=2）。 测试：`tests/test_route_goal_requires_home.py::test_an_open_resource_search_panel_is_closed_first` 由 2 个目标扩到**全部 7 个**（每个带自己的理由名，并断言面板关掉后同一个目标仍会去开门）。 A/B：去掉 brain 改动后该文件 **6 失败**（含新用例的 4 个子测试），加上后只剩 **2 个**——那 2 个是**既存红项**（`LEAVE_FOREIGN_LAYER` vs `SAFE_STOP`，见 `UnrelatedPageStillStopsTest`），与本改动无关。 **仍未修**：① 4 次"控件在帧上（d=2）却 `SEMANTIC_TARGET_NOT_VERIFIED`"的解析器层不一致；② 4 次 `OPEN_HOME_NOT_PROVEN`（3 次 after=POPUP）；③ **真机验收**：该守卫尚未在设备上跑过一轮（下一次 AUTO 走到地图且搜索面板在场时即可观察）。 **第八个发出点（2026-09-23 夜，已修）**：守卫落地（`c522bc9`，`20:41:32Z`）**40 分钟后**同类失败又成串出现 **8 次**（`21:22:27`–`21:27:57Z`），全部 `search_open=True`；其中**七次是"一步长"的运行**（开局→回城→失败→结束→面板 30 秒后重启同一状态）⇒ **六分钟死循环**，直到一次无关的 `SCAN_MAP_FOR_BEAST` 平移地图才把面板关掉。根因：`OPEN_HOME` 的发出点**有八处**——`brain.py` 里七处全绑具名 goal 且都已加守卫，**第八处在运行时**：`LiveRuntime._deferral_replan`（`runtime.py:651`，运行时唯一的 Decision 发出点），它的触发条件恰恰是"这张地图上无事可做"，而"无事可做"最常见的成因就是上一轮把面板留在地图上 ⇒ **它在最需要守卫的地方没有守卫**。已加同形状三行（`BACK` / `reason=close_resource_search_before_the_deferral_hop` / `expected_result=resource_search_closed`，复用既有 `BACK` 与 `verify_safe_back` 的 `search_closed` 半）；**刻意不设 `_replan_attempted`**（关面板是跳转的前提、不是跳转本身，那个标志保护的边界不变）。证否"这条 OPEN_HOME 不来自大脑"用的是**条件排他**：同一帧上（生产视觉 + 生产大脑）`current_goal` 为 `AUTO_DISCOVERY`/`""` 时大脑答 `SELECT_RESOURCE`，且七处大脑发出点没有一处能对 goal-less 的运行发声（`tools/probe_open_home_fallback.py`）。证据归档 `dataset/truth_audit/open_home_fallback_20260923/`。测试 `tests/test_capability_gate.py` 两个新用例（规则 + 三条边界；**直接测函数**而非跑一轮，因为"这轮有没有可选 goal"取决于机器的公平台账——该类已有两个用例正是那个 moving set 的受害者）。**顺带补上的证据缺口**：episode 流此前**不记录 `decision.reason`**（它只写进 worker stdout，随日志滚动消失，所以爆发串到底由哪个分支发出只能靠排除法）⇒ `Episode.decision_reason` 已加，下一次同类问题可直接从语料读出。 |
| 102 | **红点作为优先级信号（第二份文档 §一 / §二②）已落地：受控 A/B 处理组 8/8 换板首、负向对照 0/4；并修掉一个真实的绑定缺陷** | 🟢 **已做 + 已用真机帧回放验证（不需要设备）** | **改动三处**：① `entry_badges.dot_varies`（读表新增的 `dot_variability` 段）+ `dots_pointing_at`（只认 PRESENT × **被量到会消失**的角标 × 绑定到一个 Goal）；② `goal_utility.RED_DOT_BONUS = 60` 加进**唯一**排序层，`UtilityBreakdown` 新增 `red_dot` / `red_dot_on`，决策日志 `why()` 会写出是哪个入口在指；③ **修绑定缺陷**：面板「科技研究」行的红点原本绑到 `"RESEARCH"` —— 那是**路由域**不是 Goal id（`GOAL_ROUTES` 里 `KEEP_RESEARCH_PRODUCTIVE -> RESEARCH`），而这一行是面板里最常亮（12 帧 9 PRESENT）、也是唯一真正有区分度的行 ⇒ 面板红点信号的大半原本**落不到任何 Goal 上**（生产记录里那些行的 `goal` 字段就是 `"RESEARCH"`）。 **证据（受控 A/B，`tools/replay_red_dot_priority.py`，同帧同代码只切 `red_dots` 一个变量）**：12 张生产 HOME 帧 ⇒ **处理组 8/8 板序改变**（邮件行 180→240 登顶、面板科研行 180→240 登顶、盾兵行 80→140），**负向对照 0/4**（图上没有红点的帧一动不动，说明变化来自红点本身）。归档：`dataset/truth_audit/red_dot_priority_20260923/`（README + replay.json）。界是**算出来**的：`180 + 60 = 240 < 250`（一次可领取）⇒ 红点越不过任何付钱的目标（情报 800 / 巨兽 1000 / 体力 2450），即 §二②「红点本身不自动最高优先」。 **刻意没做**：§二①「新红点唤醒 Goal」—— 用 `tools/measure_mail_dot_blind_window.py` 量过：1085 个 HOME before-帧里只有 40 帧带邮件行读数（红点层 2026-09-22 才落地），其中 5 PRESENT / 35 ABSENT，而「读完邮件（说无事）之后 15 分钟 TTL 内红点又出现」在语料里**一次都没有出现**（唯一一次在读数后 1841 分钟）⇒ 没有证据支持为此新增一条会重复开页的规则。 **仍未做**：§二③ 保护"已到最后一步的 Goal"（目前只有 60 分界间接保证，没有独立判据）、④ 未知红点的有界探索与恢复条件、⑤ 已完成待领取的优先级（受 #95 B「领取控件未找到」制约）、⑥ 真机验收计时（"新红点从发现到首次有效操作的耗时"）；面板的 `联盟捐献 / 英雄招募 / 我的奖励` 三行在 12 帧里角标**全部 UNKNOWN** ⇒ 读不出来就没有信号（与 #100 是同一半）。 **顺带证实**：2 帧「记录说没红点」用今天的代码重读**读到了红点** —— 旧记录写在 #98 的角标窗口仍在「按钮左三分之一」时，修复移到右三分之一后同一帧就读得到 ⇒ **#98 的效果在数据里可见**（本目录对照组因此按图片分组，`summary.records_that_understated_the_dot = 2`）。 |
| 103 | **`TRY_ORDINARY_CONTROL` 的失败里 21/32 的"理由"是假的：四个闸门拒绝帧上画着的控件，执行器却一律报成"帧上没有这个控件"** | 🟢 **已修 + `ORDINARY_CONTROL_ALREADY_USED_THIS_RUN` 已真机落地（Live 证据 2026-09-23 06:15:28+08）** | **真机已验（此前未验的那一半）**：`learning/episodes.jsonl` 里已有一条真实 episode——`20260923_060723_868796` step 18（`09-22T22:15:28Z`，goal `ALLIANCE_ROUTINE`，page HOME，`quick_panel.handle` COLLAPSED 且 `point_norm=(0.0181,0.4301)` 正画在帧上）——`failure_type` 写的就是 `ORDINARY_CONTROL_ALREADY_USED_THIS_RUN`，而**修复前的同类 14 条同类失败**（近 6h，page HOME、handle 在帧上）全部被记成 `SEMANTIC_TARGET_NOT_VERIFIED`。`repo_revision` 记的是 `de55d6a`，说明运行进程导入的是**工作树里尚未提交的**那次改动，这本身也印证了「帧↔提交按时间对不上时不要下结论」。另外两个具名拒绝（`_NOT_FOR_THIS_GOAL` 等）在语料里仍只有单测实例。 **数字**：该技能全量 69 次尝试（38 S / 32 F），失败**全部**是 `SEMANTIC_TARGET_NOT_VERIFIED`，而它是近 12 小时里最大单类失败贡献（54 次里 29 次）。按"本 run 第几次尝试"分组：**第 1 次尝试 50 次里 13 失败，第 2 次及以后 19 次里 18 失败**。**根因（逐项有代码行）**：`_ordinary_control_candidate` 的声明控件层有四个闸门都会拒绝一个**帧上画着**的控件——本 run 已用过（`runtime.py:3262`）、记录不服务这个 goal（3315）、记录不列这个页（3312）、risk 不可探索（3318）——**四个都只 `return None` 不出声**，于是 `executor.py:78` 把它压成同一句话，`runtime.py` 再原样写进 episode（§6 禁止"两个根因不同的缺陷共用同一个 reason 字符串"）。**连带两处**：① `_collect_ui_evidence` 把它按"named and unlocatable"归档 ⇒ 往 `knowledge/perception/candidates/` 写了一条**假的**"读不到"候选（元素其实读到了，只是被拒绝使用）；② `escalation_queue.is_ui_unread` 按后缀匹配 ⇒ 分类成 **`UNKNOWN_UI`**（"客户端显示了 V2 读不懂的东西"）⇒ 会请开发 agent 去修一个**没有任何东西读不懂**的问题（同文件 §209-232 记着一次错误归因耗尽整个修复预算的真实代价）。**修法（只改"怎么记"，不改"点哪里"）**：① `_ordinary_declined` 每次解析开始清空（与 `_ordinary_last` 同纪律），四闸门各按名字记下拒绝，并各打一行日志；② 新 `_failure_type_from(execution)`：执行器报 `SEMANTIC_TARGET_NOT_VERIFIED` 且**本步确实什么都没解析出来**（`_ordinary_last is None`）而运行时具名拒绝时，用那个名字；③ 具名拒绝不再按 unlocated 归档；④ 名字不落 UI 后缀 ⇒ 不再被当 `UNKNOWN_UI`。名字：`ORDINARY_CONTROL_ALREADY_USED_THIS_RUN`（实测 11）/ `_NOT_FOR_THIS_GOAL`（实测 10）/ `_NOT_ON_THIS_PAGE` / `_RISK_NOT_EXPLORABLE` / `_ALREADY_OPEN`。 **结果（同一批真实帧，`tools/probe_ordinary_control_resolution.py` 复算）**：理由为假的失败 **21 → 0**；剩下 11 条仍是 `SEMANTIC_TARGET_NOT_VERIFIED`，其中**全部 11 条帧上 `handle=False`**（page=UNKNOWN 或 TRAINING）⇒ 这句话在它们身上是**真的**。**点击行为零变化**（改动不触碰 tap 决策）。证据归档 `dataset/truth_audit/ordinary_control_failures_20260923/`（README + resolution.json + report.json）。测试 `tests/test_ordinary_control_decline.py`（17 项）+ `test_quick_panel_handle.py` 两个路由用例各加一条"拒绝必须说出是哪个闸门"。 **仍未做/未验**：① **真机**——AUTO 全程在跑，改动只在离线验证（帧 + 生产解析器），"新名字真的落进 episode"要等下一轮真机出现该技能才算 Live 证据；② 三个结构性名字在语料里没有实例（只有单测）；③ **新发现的知识缺口（未修，需操作者/真机）**：10 条 `_NOT_FOR_THIS_GOAL` 说明客户端在 HOME 上画着把手而当轮 goal 是 `MAIL_ROUTINE`/`ALLIANCE_ROUTINE`/`AVOID_STAMINA_WASTE` 时解析器拒绝打开面板，但 `brain.PANEL_ROWS_FOR_ROUTE` 明确给 `ALLIANCE`/`DAILY` 也分配了面板行（`ALLIANCE_DONATION`/`MY_REWARDS`）⇒ 字典记录 `QUICK_PANEL_HANDLE.related_goals` 只列 TRAIN/RESEARCH，**比面板实际被使用的范围窄**；扩宽它 = 设备上多 10 次面板打开，属行为改动，需真机验收。 |
| 104 | **24% 的运行是纯空转：`OPEN_MAP` → `OPEN_HOME` → `OPEN_MAP`，两个方向各自都对、合起来是个环** | 🟢 **已修（runtime + 13 项测试；真机帧受控 A/B 36 步 → 27 步；真机未跑到）** | **数字**：`learning/episodes.jsonl` 自 16:00Z 起 75 个运行 / 597 步，其中 **18 个运行（36 步）全部只由 `OPEN_MAP`/`OPEN_HOME`/`CHECK_MARCH` 组成**；`goal=AUTO_DISCOVERY` 的 91 步里 **79 步是导航**，其中 `OPEN_MAP` 44、`OPEN_HOME` 25、`CHECK_MARCH` 10。 **链条（回放，不是推断）**：`tools/replay_run_decisions.py`（生产视觉重建世界 + 生产大脑按顺序喂进同一个 run 实例）对 `20260923_062948_568906` **3/3 精确复现** —— `22:30:26 HOME→MAP OPEN_MAP`（`brain.decide` / `first_ready_p0_skill`）→ `22:31:28 MAP→HOME OPEN_HOME`（`runtime._deferral_replan` / `deferred_SPEND_STAMINA_ON_BEAST_left_nothing_to_do_here`）→ `22:31:58 HOME→MAP OPEN_MAP`（与第一步**逐字相同**）。即 `brain` 的无目标兜底（「地图上才有目标可看」）与运行时的兜底跳转（「回城才能读到队列目标」）**各自正确、互为反向、互不知情**。 `_replan_attempted` 的注释自称「so leaving cannot become a two-page ping-pong」——实测做不到：**只给双向环的一侧上界，环还在**；缺的不是第二个界，是记忆。 **修法**：`LiveRuntime._stop_instead_of_looking_again` + 运行内 `_barren_pages`（站过且调度器什么都没给的页面）+ 类常量 `PAGE_HOPS_THAT_ONLY_LOOK_FOR_GOALS`（`OPEN_MAP`→MAP、`OPEN_HOME`→HOME，即「只为找目标而存在的两个跳转」）⇒ 目的地已在荒页里就换成 `SAFE_STOP` / `every_page_this_run_was_fruitless`（已登记进 `runtime_snapshot.NON_FATAL_STOPS`）。四条边界各有测试：只判**无目标**步骤（带目标的跳转原样返回，INTEL 路由发的 `OPEN_MAP` 不受影响）、只判那两个跳转、站着的页面**无论是否是跳转**都记荒页（在地图上读 `CHECK_MARCH` 也是同一个发现）、答案是**停止**而非另一个跳转。 **A/B（同一批真实帧，只切守卫一个变量）**：18 个空转运行 / 36 个记录步骤 ⇒ 回放与语料一致 **28/36**（不一致的 8 步是生产里当时**有**目标的步骤，本回放只重建无目标链，工具文档写明）；**加守卫前 36 步 → 加守卫后 27 步**，**7 个运行提前诚实停止**；两个方向都被命中（`20260923_000958_683601` 4 步→2 步；`20260923_000639_research` 3 步→2 步），逐条见 `dataset/truth_audit/nav_pingpong_20260923/console.txt`。 **顺带**：episode 流此前**不记 `decision.reason`**（#103 那轮已补 `Episode.decision_reason`），而运行时日志只留最近一轮 ⇒ 这三个理由本来**没有幸存证人**，只能从帧回放拿到。**（当日更正）**：`decision_reason` 已于 `09-22T23:31–23:39Z` 首次出现在语料里（rev `7da38a6`，如 `alliance_gifts_badge_visible` / `mail_goal_leaves_a_panel_it_does_not_own` / `first_ready_p0_skill`）⇒ **下一轮同类问题可直接读语料，不必再回放**；本 issue 的回放仍然是这一轮唯一的证人（那些步骤写在字段落地之前）。 **仍未做/未验**：① **真机**——AUTO 每 30 s–10 min 起一轮，改动只在离线验证；判据是下一次「无事可做」的运行里 `episodes.jsonl` 的 `decision_reason` 出现 `every_page_this_run_was_fruitless`；② 有目标的跳转（INTEL 要地图、训练要回城）是否也会绕圈**未查**，需要各自目标的帧；③ 回放只看记录里的 before 帧，缺帧的那一轮（`20260923_001936_231102` 的 #2）看不到跳转结果，是回放的限度而非守卫的限度。 |
| 105 | **episode 的 `goal_id` 字段里混着两套词汇表（Goal id 与路由域），使"红点落点"之类的归因出现假一致** | 🟠 **已量化、已登记；不改行为（改的是分析口径）** | **实测**：`runtime._step_goal` 在 `best_goal is None` 时返回 `self.brain.current_goal` —— 而那是**路由域**（`ROUTE_DOMAINS` 里的 `RESEARCH`/`TRAIN`/`MAIL`/…），于是同一个字段有时写 `KEEP_RESEARCH_PRODUCTIVE`（Goal id）、有时写 `RESEARCH`（路由域）。**后果**：核对"红点指向的 Goal 是否就是这一步选的 Goal"时，6928 步里有 **10 步**看似一致，其中 **9 步是假的**——红点当年绑到字符串 `"RESEARCH"`（#102 修的正是这个绑定缺陷），而 episode 的 `goal_id` 也可以是字符串 `"RESEARCH"`，两者**碰巧同名**。⇒ 任何跨这两张表做的统计，必须先声明用的是哪一套词汇表；红点侧的记录请一律经 `entry_badges.QUICK_PANEL_ROW_GOALS` / 表里的 `goal` 归一到 Goal id，不要直接与 `goal_id` 字段比对。 **为什么登记而不是修**：`_step_goal` 的"提交标签"语义是有意为之（见其 docstring，它救过 `KEEP_MARCHES_PRODUCTIVE`），把路由域从该字段拿掉会改动已归档证据的标签；而真正该做的是让**分析**不混用词汇表（本轮新增的测量都在红点侧归一后再算）。这也是红点#102 那条"绑定必须对着下游的词汇表断言"的同一族问题，只是这次撞的是**上游**。 |
| 106 | **§一 审计完成：AI 建议没被执行，第一个断点是"可达性"，不在执行链里** | 🟠 **已审出并已离线证否执行侧**（修法未落地） | **对象**：盘上**唯一**一条真实答案 `unknown__control__b6546e80`（AUTO 于 `09-22T08:31:06Z` 在 `UNKNOWN::燃霜矿区` 上问，goal=DAILY；答案 `10:12:39Z` 落盘，建议 `ORDINARY_CONTROL[奖励入口]`）。**总量：6 个提问 / 1 个答案**（另 5 个被 45 分钟 timebox 取消或排队未派发）。 **执行侧逐闸门复算**（`tools/probe_advice_replay.py`，生产视觉 + 生产 OCR，把答案放回它自己的帧）：`take()` 取到答案=True、`staleness=(fresh)`、锚点 `奖励` 在本帧 OCR 上、风险闸门无拒绝、`_advised_control` **返回落点 `[0.8819, 0.2908]`** 且 `_last_advice` 置位 ⇒ **消费侧没有断点**。 **第一个实际断点＝可达性**：`request_id` 由 `page_key` 哈希而来（答案**按屏命名**），而 `take()` 只在 `_advised_control` 里被调用，该方法只在**当前帧就是那一屏**时才会被走到；实测 燃霜矿区 记录 `attempt_count=0`、`last_seen_at=08:31:07`（**第一次被看到的那一秒，此后再没访问过**），`ai_advice` 在 16 条屏记录与 6900+ 条 episode 里**各 0 次**。且**没有任何东西**把"某屏有待用答案"变成一次可调度访问（`pending()` 的调用者只有 dispatcher/worker/tools，调度层不读）。 **修法（本轮选定，未落地）**：把"某屏有一条待用答案"接到**现有** Goal/访问(sweep)机制上变成一次访问——不新增第二套调度器、不改红点语义；落地前先在真机验证 访问→取答案→落点→MAA 点击→后帧验证。 **顺带发现的脆弱点**：id 参与哈希的是 **OCR 读出的标题**，同一屏若某次读成别的字（盘上同时存在 `常规活动` 与 `燃霜矿区` 两条记录）⇒ id 变、答案被孤立。证据归档 `dataset/truth_audit/advice_execution_20260923/`。 |
| 107 | **红点三层次语义（新指令 2026-09-23）：通知 / 入口控件 / 页内动作——代码现状已成立，缺的是"AI 建议必须区分四件事"** | 🟢 **语义已审计 + 已钉成不变量测试** | **审计结论**：「红点只是通知信号，不是点击目标」在代码里**已经成立**：`EntryBadge(entry,page,state,observed_at,goal,task_state,pixels,box_norm,reason)` **没有任何 point/centre/tap 字段**，`box_norm`/`pixels` 只是读数证据；`red_dots` 的消费点只有两处——`goal_utility` 的**打分项**与 `brain.py:809` 的**分支**（读 ABSENT 就不进邮件页），**没有一处把红点当坐标**；入口是**具名控件**（`OPEN_TASK_FROM_QUICK_PANEL_<ROW>` 四行俱全，三行兵营走共用 `OPEN_POWER_OVERVIEW → NAVIGATE_*_CAMP → OPEN_*_TRAINING`）；页内动作由目标页读数 + 各自 verifier 决定。 **已钉成不变量**：`tests/test_red_dot_is_a_notification_not_a_target.py`（8 项 / 33 subtests）——① `EntryBadge` 不得出现类坐标字段；② **"消费点只有排序层"**（`dots_pointing_at` 出现第三个消费者即失败，强制后来者显式声明）；③ 每个面板行必须有具名入口控件；④ 无帧读数必须是 `UNKNOWN` 而非 `ABSENT`（面板关闭/行未显示/识别失败 ≠ 确实没有红点）。 **缺口（未改）**：`Advice` 有 `proposed_action/action_kind/target_semantics/candidate_semantics/expected_result`，但**没有**"这条建议是关于哪个通知/入口"的字段 ⇒ "AI 建议必须区分 通知/入口/页内动作/业务目标"目前靠字段约定而非结构表达。 **真机验收清单**（必须**分别**证明，不得用"点击红点附近"冒充"红点任务已处理"）：notification 是谁 → 它影响了哪个 Goal 选择 → 实际选了哪个 Goal → 执行了什么动作 → 后帧变化 → 奖励是否实际到账。 |
| 108 | **限时活动（新指令 2026-09-23）：能力已存在、断在接线——`EventGoalPlanner` 的调用者只有单测，`runtime`/`brain` 一次都没调用** | 🟠 **已核实（本轮只审计+登记，未接线）** | **已存在的能力（不要重写）**：`winter_agent_v2/event_goal.py`（`DeadlineLevel`：<2h→P0 / <6h→HIGH / <12h→ELEVATED；`EventState`；`EventPlan` 含 `feasible`、`chosen`、`expected`、`CLAIM_REQUIRED`；`EventGoalPlanner.plan()`）；`goal_library.GoalState.remaining_seconds` + **`deadline_pressure()` 已经进唯一排序层**（`priority` 第一项）；`discover()` **已经在读** `world.events`（`minimum_guarantee.remaining_seconds/estimated_cost` → 一个 reward 500 / synergy 500 的目标；`bear.remaining_seconds|seconds_to_start` → 另一个）。**⇒ 指令 §三"根据剩余时间提高其现有 Scheduler 优先级"这一半已经在跑。** **第一个断点＝接线**：全仓库 `EventGoalPlanner` 的调用者**只有 `tests/test_event_goal.py`**；`runtime.py`/`brain.py` 对它**零引用**；控制面板会**显示**它（`tools/control_panel._event_goal`），但**执行链不用它** ⇒ 缺的是"哪个活动 Goal 现在该被唤醒、剩余时间内哪些任务可完成、该领奖了就去领奖"这一半。 **与"30 分钟活动"直接相关的两个量化缺口**：① `deadline_level()` 最紧一档是 **<2 小时**，30 分钟落 P0 没错，但**没有分钟级档**，"剩余 8 分钟"与"剩余 100 分钟"同级；② `EventPlan.feasible`（剩余时间够不够完成）**从未被消费**，所以"启动一个明显做不完的流程"目前无人拦。 **修法（不新增 Brain/Scheduler/执行器）**：把 `EventGoalPlanner.plan()` 接到现有 Goal 的优先级与 `next_action` 上 —— 活动 Goal 由现有 `discover` 产出（已成立），提权由现有 `deadline_pressure` 承担（已成立），只补"计划/可行性/领奖"这一段的消费点。 **真机验收（§八，选下一次真实开放的活动）**：开放前知识准备 → 开放后多久首次有效操作 → 活动期间优先级是否真的压过采集/红点/日常 → 未知操作能否按当前 UI 首次尝试 → AI 建议能否在活动仍有效时进入 MAA → 完成后是否及时领奖 → 结束后是否恢复其他 Goal。 |

| 109 | **一个坐标被当成"所有 POPUP 都适用"：退出确认弹窗上学到的 X 坐标，8 轮以上被花在加成总览上（点面板中部）；且 X 会动而模板层只在"核对它还在原位"** | 🟢 **已修（两半：台账按屏存 + 给 X 一条会搜索的记录）；离线取证完整，真机未验** | **现场**：`09-23T05:23:10Z..05:51:42Z` 共 **82 次** `CLOSE_POPUP` → `POPUP_CLOSE_NOT_PROVEN`，`executor_backend.jsonl` 里每次 `tap_point` 都是 **`[635,456]`**；帧上是加成总览，它自己的 X 在 **(666,157)** ≈ (0.925,0.123)，而 (635,456) 落在面板自己的数字列上；点完后帧同屏（`after_popup` 仍 `POWER_OVERVIEW`）。**根因**：`_resolve_semantic_target` 的链条是 模板→`_client_printed_control`→**`_remembered_control_center`**→字典提示；实测今天用生产 `find('BTN_CLOSE')` 跑这些帧返回 **None**（四个 phash 记录 d=28/34/40/34，阈值 8）⇒ 落点是**台账**给的（`POPUP|BTN_CLOSE` = `[0.88194, 0.35625]` ×(720,1280) = (635,456)，`attempts=97`，`known_change=NUMBER_CHANGED`）。而 0.8819/0.3563 属于**退出确认**：四个注册各自的父帧读出来是 `PURCHASE_POPUP`/`PURCHASE_POPUP`/`EXIT_CONFIRM`/`ALLIANCE`（`tools/probe_close_popup_target.py --survey`）。`control_key` 只有 `(page, semantic)`，而 `POPUP` 底下有 **22 种**叠层、**2717/2717 条读数都带着是哪一种**（GET_MORE_STAMINA 289、GENERIC_REWARD 281、POWER_OVERVIEW 218…）⇒ **25% 的屏身份（3507/13883）被丢掉**（另有 ALLIANCE section、TRAINING 营别、MAP target）。**修法一（台账按屏）**：`control_key(page, control, screen="")` 仅在 screen 比页面窄时用它（`POPUP|POWER_OVERVIEW|BTN_CLOSE`），页面即屏幕时键**一字节不变**（57 条位置里的 49 条）；新增 `ControlExperience.screen`（与 `position_norm` 同一处写入）+ `reusable_on_this_screen()` 作第二道守门（实测：主力是键，不同弹窗压根查不到；守门只在键与记录的屏走散时生效——这一点原先被我写成"守门在拦"，测试当场纠正）。运行时三处调用点同步。**修法二（X 会动就得找）**：给 `BTN_CLOSE` 加**一条** `ccoeff`+`search_band` 记录（x0.80/y0.07/w0.20/h0.36，由 6 个实测中心反推），复用**已有已复核**的 X 模板；`tools/register_close_popup_band.py` 每次重算分离度、不达标拒绝写入：**画出 X 的 8 帧 distance=[0,0,0,0,0,1,1,1]，没有 X 的 10 帧=[23,27,27,27,27,29,29,30,32,35]**，阈值 8 落在空隙正中 ⇒ **不需要动任何默认值**。**解析结果**：`POWER_OVERVIEW → (0.9236,0.1301) d=1`、`EXIT_CONFIRM → (0.8819,0.3559) d=0`（原位未坏）、`GET_MORE_STAMINA → (0.9236,0.1129) d=1`、`GENERIC_REWARD → None`、`HOME/MAP → None`。**回归**：9 个台账/调度文件同运行器 A/B（`learning/_ab_screen_key_plugin.py` 把键退回页面大类）= 基线 **15F/212P** vs 本树 **8F/219P**，**新增失败 0**，基线多出的 7 条正是本轮新写的测试（无修时按设计失败）；`BTN_CLOSE` 相关 11 文件在"摘掉新记录"（`AB_CLOSE_BAND=off`）下逐条相同；`check_wiring` 9 项逐条相同。证据 `dataset/truth_audit/close_popup_20260923/`（README + survey.json + probe.json + 复核图）。 **未做/未知**：① **05:54:23 那次失败类型的切换不是本轮的功劳**（本轮第一处编辑落盘 05:57:44，比它晚 3 分钟；台账条目当时仍在且 `resolved`，按修复前代码本应仍给出那个点）⇒ **成因未定，不认领**；② **真机未验**：本轮只证到"解析给出的点是那个 X"，"弹窗真的关掉、后帧不再是 POPUP"要下一轮真机看 `CLOSE_POPUP` 成功；③ `POPUP|*` 下另外 7 条控件位置（`BTN_OPEN_POWER_DETAILS`/`BTN_DISMISS_INTEL_REWARD`/`BTN_INTEL_VIEW_TARGET`…）键变窄后不再跨屏误用，但**没有各自的 band 记录**。 |
| 109-LIVE | **#109 的真机确认：记录落盘 31 秒后，`加成总览` 上 `CLOSE_POPUP` 首次成功——死循环断掉** | 🟢 **LIVE_VERIFIED** | 时间线：`06:02:52 / 06:03:28 / 06:04:05 / 06:05:22×4` 全部 `SEMANTIC_TARGET_NOT_VERIFIED`（修法一已在工作树里，不再有错误点击）→ **`06:05:51`** 注册 band 记录 → **`06:06:22` `CLOSE_POPUP` SUCCESS**。执行台账：`tap_point [665,167]`（旧值恒为 `[635,456]`，那是面板自己的数字列），`executed=true`、`error=null`；episode：`state_after page=HOME, popup=null`，`verifier_evidence={"before_popup":"POWER_OVERVIEW","after_page":"HOME","after_popup":null}`。解析点 `(0.9236,0.1301)×(720,1280)=(665.0,166.5)` = 实际点击点 = 帧上目视量到的 X，三者同一位置。**"首次"可核对**：56 次 `CLOSE_POPUP` 成功里 55 次在 `EXIT_CONFIRM`，`POWER_OVERVIEW` 上此前 **0 次**（82 次失败）。该步 `repo_revision=5ce5f5ae…+86852481…`，加号后是**工作树脏哈希** ⇒ AUTO 跑的是当时未提交的代码，"落盘时间"要按文件 mtime 算。证据：`dataset/truth_audit/close_popup_20260923/`（`live_success_before/after_20260923T060602.png` + `live_success.json`）。**仍未覆盖**：其余 21 种弹窗（`GET_MORE_STAMINA` 289 帧、`GENERIC_REWARD` 281 帧…）没有人见过 `CLOSE_POPUP` 成功；`POPUP|*` 下另外 7 条控件位置没有各自的 band 记录。 |
| 110 | **`BTN_OPEN_POWER_DETAILS`（加成总览的「实力详情」）14 帧全失败：客户端把图标和名字印成**一个 token**（OCR 读作 `三实力详情`），精确匹配读不到；两条 phash 注册正好落在按钮画的位置的空隙里** | 🟢 **已修（读客户端印的名字，按记录声明允许"名字在 token 内"）；离线 14/14 + 负向 18/0，真机未验** | **实测**：该技能历史 113 S / 22 F，全部在 `POPUP|POWER_OVERVIEW` 上；两条注册 roi 中心 (360,797)/(360,660)，即 y 636..685 与 y 762..832，而按钮画在 **y ~692..765（空隙里）**，phash 距离 36/33 vs 阈值 8 ⇒ 模板层 0 匹配。**OCR 其实读得到**：整帧 25 个 token 里 `三实力详情` conf **0.901**，中心 (360,728) = (0.501,0.568)，与按钮实际中心 (357.5,727.5) **差 3 px**。**根因**：客户端在那颗蓝色药丸上同时画 ≡ 图标与名字，OCR 合并成一个 token；而 `find_printed_words` 默认**精确**匹配（该默认有实测依据：`城镇` 曾命中 `我的城镇`），`allow_containment` 参数存在但**生产无调用者**。**修法（不是再加定位器）**：把"名字可能被并进更长的 token"做成**按记录声明**的字段 `ocr_merged`（写入词典 `schema_note`），`_declared_record` 返回 `(pages, words, merged)`，`_client_printed_control` 接到读取器的 `allow_containment`。**只有被实测到会合并的那一条记录**放宽（全词典仅此一条 `merged=True`）。**结果**：14/14 失败帧解析到 (0.5007,0.5684)=按钮中心；**18 屏负向对照 0 命中**（含 8 种其他弹窗、HOME/MAP/TRAINING/UNKNOWN 等）；覆盖率 **231/245 = 94.3%**（14 帧 OCR 读不到 ⇒ 仍会诚实失败，这是该路线的边界）。默认精确行为未动，并有测试钉住。**未验**：真机下一轮应看到该技能在 `POWER_OVERVIEW` 上成功、后帧为 `POWER_DETAILS`。证据 `dataset/truth_audit/power_details_entry_20260923/`（`printed.json` + 逐帧复核图）；工具 `tools/probe_printed_control.py`（通用：给定词，量任意语义/任意屏的正负命中）、`tools/register_power_details_record.py`。 |
| 111 | **弹窗里同一个决定被每个 goal 重新推导一次：`blocking_popup` 先于一切 goal 路由，于是"让位给下一个任务"只换回同一步** | 🟢 **已修（有界 2 次 + 让位只给一次）；离线已证，真机未验** | **实测**：运行 `20260923_140514_506593` 在 **3 秒内 5 步 CLOSE_POPUP**，五个**不同 goal**（CLEAR_INTEL / MAIL_ROUTINE / DAILY_ACTIVITY_TARGET / CLAIM_EXPLORATION_IDLE / AUTO_DISCOVERY）却是**同一个决定**——`brain.py:777` 的 `POPUP` + 有名字弹窗分支在任何 goal 路由之前。全语料 1535 个运行里 `≥4` 连续弹窗步只有这 1 例；最长连续 POPUP 步分布 {1:705, 2:134, 3:8, 5:1, 13:1}；"同一(运行,弹窗)重复失败"多为 2，最大 11（旧运行）。**为什么现有守卫拦不住**：`_failed_controls`（§七.3 "已失败不再点"）只在**验证器失败**时写入，而这里**根本没解析出点、没跑验证器**。**修法**：新增 `_unresolved_controls`（目标 → (次数, 最近原因)）+ 类常量 `MAX_UNRESOLVED_ATTEMPTS = 2` + `_unresolved_handed_over`（让位只给一次）。**界为什么是 2 不是 1**：每次尝试都对**新抓的帧**解析，而本项目最信的字读者单帧只有 ~94% 可读 ⇒ 第二次是真机会。**验证**：`tests/test_unresolved_control_is_not_rederived.py`（**不桩大脑**，用真 `RuleBrain` 在 `POPUP|POWER_OVERVIEW` 帧上确实答 `blocking_popup`）同时跑 bound=2 与 bound=10⁹ 两腿 ⇒ 有界时 **2 次尝试**、无界时 **>2 次**（负向对照）。A/B（9 文件、同运行器、`AB_UNRESOLVED_GUARD=off`）：**两侧 11 failed 逐条同名** ⇒ 零回归，且 11 条全在三个已知红项文件里。**仍未做**：真机确认（下一轮若再遇不可关弹窗，判据是同一目标最多 2 次失败、随后本轮带真原因结束）。 |
| 112 | **AI 兜底通道：答案不是"没被采用"，而是没有任何东西把它送回被问到的那一屏；且被反复提问的题会被永久孤立** | 🟠 **两处已修（离线可复算）；真机验收进行中** | **操作者 2026-09-23 §一：交出 `AI request_id → 自动答案 → AUTO 采用 → MAA 动作 → 后帧变化` 的真实 episode。** **实测（`tools/probe_unknown_channel.py --revisits`，全部来自磁盘产物）**：① **7 个提问的屏，在答案落盘后的 41 次 UNKNOWN 步里，被问的那一屏回来了 0 次**——`UNKNOWN::燃霜矿区` 最后出现即提问那一刻（`09-22T08:31:07`），而答案 `09-22T10:12:39` 才落盘，**晚了 101.5 分钟**；判据是**逐帧 OCR 标题**（我第一版用页记录的 `last_seen_at`，那是错的：它只在 `stage`/`record_attempt` 写，一次什么都没解析出来的重访不更新）。② **7485 条 episode 里带 `ai_advice` 的 0 条**，而站过 UNKNOWN 屏的步骤 145 条 ⇒ 消费点从未在"有答案"下运行过。③ 通道台账 13 `submitted`/15 `reconciled`，只有 1 条题有答案。 **缺陷一（`unknown_dispatch.reconcile`）**：**先看 job 状态、后看产物**。job `13ffdac6` 在 `10:12:39Z` 写出答案，台账在 `10:37:49Z` 记成 `ABANDONED / no result after 45 minutes` **并把它取消**。后台会话干完活仍显示 `working` 直到网关回收 ⇒ 年龄时钟把"已产出"记成"什么都没产出"，模型路由需要的成功样本也从未记录。修法：**只在非终态分支**先看产物（`state: "ANSWERED"` + 网关自己的 `verdict`），终态分支一字未改。 **缺陷二（`unknown_dispatch.candidates`）**：`MAX_ATTEMPTS_PER_REQUEST` 被当成**终身**上限。`unknown__control__78694f1b`（屏 `UNKNOWN::对战`）两次尝试在 `09-22T19:41` 花光，运行时仍在 `09-23T08:30` 重新提问（`ask` 每次访问该屏都重写请求文件）**却再也提交不了 job** ⇒ 通道悄悄停止回答一个它仍在被问的问题；而那一屏 **23 小时被站过 20 次**。修法：`retry` 判据改为"**自我们上次放弃以来运行时又问过**"（`request.created_at` 比 `record.updated_at` 新），再问节奏由运行时自己的 1 小时冷却限着；另加 `orphans()` 把"只能等人"的问题**说出来**。 **验收目标**：`unknown__control__78694f1b` / 屏 `UNKNOWN::对战`（帧上仅 5 词：`对战/53.6万/45.3万/自动/X2`）/ goal `INTEL`——与真机到该屏时的 `current_goal` **一致**（决策原文 `unnamed_page_with_goal_INTEL_may_still_name_an_ordinary_control`），且是唯一"屏会反复回来"的开放题。**燃霜矿区那条答案按操作者要求不强行执行**（该屏只出现过一次、领奖窗口已关），知识留在盘上供下次活动。 **离线确认**（`replay.json`）：真机帧今天仍读出 `UNKNOWN / 对战`，算出的 `request_id` 与答案文件名**完全一致**，解析链要的 5 个区域都在，`_advised_control` 只因"还没答案"返回 `None` 并如实提问。 **测试**：`tests/test_unknown_ai_channel.py` 29→32（两条新行为 + 反向对照"没有产物的卡死 job 仍被放弃且仍被取消"）；`check_wiring` 9 项逐条相同；`test_live_runtime.py` 3 条是该文件单跑就有的既存红项。证据 `dataset/truth_audit/ai_fallback_20260923/`。 **⚠ 未决**：① **真机 episode 尚未取得**（job `06066de6` 于 `12:17:56Z` 提交、`tempo=active`，等答案 + 等 `对战` 屏再次出现）；② **面板是长驻进程，`unknown_dispatch.py` 不在 `CONTROL_PLANE_PATHS` 里** ⇒ 本修复要等窗口重启才对**面板自己那一遍**生效（这一次是我用 `tools/unknown_ai_worker.py --once` 走的一次合法手工 pass，台账里 `reason: retry 3` 可见）；③ 另有**潜在**身份缺陷（未被真机触发）：`advice_staleness` 读的是**当前**请求文件的 `goal`，而 `ask` 会重写它 ⇒ 一份"为 goal A 回答"的答案**可能**在请求被改写为 goal B 之后被当成新鲜的采纳。仅由读码得出，无真机实例，**本轮未改**（属安全门，不擅自放宽）。 |
| 113 | **`OPEN_POWER_DETAILS` 真机复核（操作者 §二）：点击与后帧都对，新断点是"页面识别"——整面板 phash 差 **2** 个距离单位** | 🟠 **已定位（有真机直接证据）；仪器未注册（分离度未量干净）** | **真机（run `20260923_201923_535962`，rev `8e8489f`）**：`12:24:41Z OPEN_POWER_OVERVIEW SUCCESS`（HOME→POPUP/POWER_OVERVIEW）→ `12:24:42Z OPEN_POWER_DETAILS` 执行台账 **`tap_point=[361,728]`**、`used_backend=ADB` → `12:25:24Z` episode `FAILURE / POWER_DETAILS_NOT_PROVEN`，`state_after page=UNKNOWN popup=None`，`verifier_evidence {"overview_before": true, "details_after": false}`。**① 点击对不对：对。** 上一轮在真机帧上量到的按钮中心 `(357.5,727.5)`、OCR 标签 `三实力详情` 中心 `(360,728)`、实际落点 `(361,728)`，三者 ≤3.5 px。**② 后帧进没进：进了。** `step_014_after_refresh_2_20260923T122512050579.png` 帧上就是「实力详情」：大熔炉18 / 总实力 2,004,198 + 建筑/部队/英雄/英雄装备/科技/宠物 六行 + 每行「提升」。⇒ **动作成功、后帧变化正确，只是没被读成 `POWER_DETAILS`**（`verifier.py:417` 要求 `after.popup == "POWER_DETAILS"`，而 `popup` 由 `vision.py:1655` 的 `if match("POPUP_POWER_DETAILS")` 给出）。 **新的第一处断点**：整面板 phash `popup_power_details__live_20260908_power_details__0` 在这帧上 **d=10**（阈值 8，差 2）；该面板的读数每帧都变（`总实力 2,004,198`、`635,163/907288`、`超过了本王国46%的领主`）⇒ phash 每次都会偏。标题栏模板（`..._20260917_titlebar`）phash d=24，是因为**面板纵向位置会动**（今天标题 y=203，模板 ROI y 246..322；6 张历史帧标题 y 约 284）。**为什么没有顺手注册标题栏 band**：弹窗标题栏是"蓝条+居中白字"，**条本身不具区分度**——标题栏模板用 ccoeff 在带内搜索，真阳性 0.9764（d=2），而**真负样本「欢迎回来」0.8761（d=8）、`加成总览` 0.8535（d=9）**，阈值 8 恰好落在真负样本上 ⇒ 照此注册会把**离线收益弹窗**认成实力详情。整面板模板换 ccoeff 亦自相矛盾（8 张真样本 0.4933 vs 被误读那帧 0.8551）⇒ **相似度分数在此不能替代测量**，按操作者"不得把最像的位置当成存在"的要求，**不发布未验干净的仪器**。 **下一轮**：用**可复核裁片**重测正负样本再注册（候选：只含标题字形的裁片；或面板内"六行实力行"这一恒定结构）。⚠ 顺带一个新读数缺陷：`page_knowledge.read_title_candidate` 在这帧上返回 **`平庸`**（conf 0.9986，右对齐评价标签）而不是标题栏的 `实力详情`（conf 0.995 @(361,203)）。证据 `dataset/truth_audit/ai_fallback_20260923/README.md` 附节。 |
| 114 | **邮件 / 联盟宝箱仍按周期生成并进入：两条故障的**第一实际断点不同**——一条在 Goal 层（周期票），一条在 Brain（根本没有闸门）** | 🟢 **已修（三处门控 + 出口保护）；离线与语料已证，after 真机窗口刚开** | **指令**：红点对这两项不是加分项，而是**准入信号**（无红点就没有这个 Goal）；§三 父入口≠宝箱子入口；§四 UNKNOWN 不是"进去看看"；§六 出口保护；§八 量化。 **断点一（邮件，Goal 层）**：用真 `GoalLibrary` 复算——入口 `ABSENT` / `UNKNOWN` / **完全没有红点账本** 三种情形下 `discover()` **都**产出 `MAIL_ROUTINE / DISCOVERED / available_skills=('OPEN_MAIL',)`，票值只随年龄变（未读过 `SWEEP_NEVER_VALUE`=180、过期 3× 为 155）⇒ **是 Goal 仍被周期生成**，不是 Scheduler 没过滤，也不是 Executor 绕过。红点此前只经 `goal_utility.red_dot_bonus` 作 **+60 排序分**（正是指令否掉的形状）。 **断点二（联盟宝箱，Brain）**：`brain.py` 在联盟页 `section=="HOME"` **无条件** `OPEN_ALLIANCE_GIFTS`，理由字符串 `alliance_gifts_badge_visible` **声称有角标而没读任何角标**；真机语料（97 次进入）**宝箱瓦片自身角标 93 次 ABSENT**、进入前 `alliance.status=UNKNOWN` 96/97。 **改法（三处，都是"是否允许生成并执行"）**：① `entry_badges.ENTRY_GATED_GOALS` + `entry_gate()` → PRESENT 许可 / ABSENT 拒绝 / **UNKNOWN 两者都不是**（未登记入口的目标一律 UNKNOWN，不会因为被写进映射就受门控）；② `goal_library._append_panel_routine` 的周期票受门控：**ABSENT 与 UNKNOWN 都不发目标**（从候选集剔除，不是降权）；③ `brain` 邮件只在 PRESENT 开页（原来 UNKNOWN 与缺账本都开）、宝箱由**瓦片自身**角标决定；④ `scheduler.ENTRY_GATED_SKILLS` 作 **§六 最后出口**——`Scheduler.tick` 是**所有**决策通往执行器的唯一通道，因此拒绝无法被任何旧路径绕过；拒绝而**不纠正**，结算为"当前无新事项"（`SAFE_STOP` 非致命，让位其他 Goal）。 **§三 的实测依据**：联盟入口 `TAB_ALLIANCE` 是**常量**（26/26 在位，表里已记 `varies:false`）⇒ 不能决定任何事；联盟页按**瓦片**画角标（联盟战争 2 / **联盟宝箱 18** / 联盟领地 1 / 联盟商店 有点）。新增 `TILE_ALLIANCE_GIFTS`（窗口取自已人工复核的 `RED_DOT_ALLIANCE_GIFTS_18` roi 外扩，对照窗口读相邻瓦片），`tools/measure_alliance_tile_badge.py` 在 **316 帧**上量得 **PRESENT 8 / ABSENT 308** ⇒ 会变故可注册；工具**不达标拒绝写入**。 **§八 before 数字**（`tools/measure_red_dot_gating.py`，7106 步）：`OPEN_MAIL` **97** 步（入口 `NO_READING` 83 / `PRESENT` 14）；`OPEN_ALLIANCE_GIFTS` **97** 步（瓦片 `PRESENT` 4 / **`ABSENT` 93**）。**after 窗口刚开，未填**——判据：无红点时应**没有**这两步，且改为 `SAFE_STOP` + reason 含 `entry_badge` / `refused_at_the_entry_gate`，AUTO 继续训练/科研/采集。 **回归**：同运行器、模块边界 A/B（`learning/_ab_screen_key_plugin.py` 的 `AB_ENTRY_GATE=off` **逐站点复现旧语义**，而不是"一律 PRESENT"的近似）= **本树 100 passed / 0 failed**，去掉改动 **10 failed**，其中 **7 条正是本轮新写的测试**；含调度器的 9 文件批 **154 passed / 0 failed**；`check_wiring` 回到基线 **9** 项（其中 `ui: navigation matrix` 一条是既存待重算项）。 ⚠ **未做**：§五 的 `badge_generation` / `last_processed` 状态库、§七 处理成功后回到入口复读并**显式休眠**——当前这两个效果由"每帧读入口 + 领取后页面自己变回无角标"自然达成，但**没有显式的世代账本**，因此"ABSENT→PRESENT 才唤醒"这条在实现上靠读数而非靠版本号。 |
| 115 | **任务存在性只从当前帧读出来，而执行资格完全无视窗口——同一个函数里两处相反方向的混淆；外加"等待"被记成"完成"** | 🟢 **已修（4 处，都是"是否允许生成并执行"）；离线与真机证据齐，`SCHEDULED_BEAR_HUNT` 已在真机目标板上** | **指令**：先判断任务是否存在及是否具备执行条件，再排序；§一 六种语义优先映射到已有状态；§二 周期活动窗口；§三 提前准备；§六 不无意义等待。 **Q1 断点（`GoalLibrary.discover`）**：**(a) 存在性只从当前帧**——`world.events` 全项目**只有一处**写入（`ocr.py`，写当前页印的东西），而 `PARTICIPATE_BEAR`/`EVENT_MINIMUM_GUARANTEE` 只在该字段有值时生成；实测 **7156 条 episode 里只有 4 条带任何事件读数、0 条带 `bear`** ⇒ "没站在印它的那一屏" 与 "任务不存在" 是同一个世界状态（无准备、无等待对象、无"窗口关闭"记录）。**(b) 执行资格无视窗口**——`bear_phase` 早已分出 SCHEDULED/PREPARING/READY/ACTIVE/FINISHED，而生成处把**除 FINISHED 外的所有相位**都写成 `READY`；真库实测：**两小时后开的活动 = `READY` priority 5000**（压过随年龄涨到 180 的巡检票、与 `CLEAR_INTEL` 500 平级），**已结束 = `COMPLETE`**（= §一的"本次任务实际完成"）。**(c) 等待被记成完成**——队列忙 / 兵营在训练 / 行军槽用尽都曾写 `COMPLETE` ⇒ "客户端还不让我开工" 与 "没有要做的了" 是同一条记录，§六 的"记录具体恢复条件"无处可记。 **(d) 顺带查出**：第 5 轮的准入闸门放在"无有效读数"分支里，漏掉**复用读数**那条路——实测入口 `ABSENT` + 新鲜存储 `{"status":"CLAIMABLE"}` 时仍发 `MAIL_ROUTINE/READY` 及领取技能 ⇒ 目标存在、能赢板、执行不了。 **改法（4 处）**：① `GoalStatus` 补 `SCHEDULED_NOT_OPEN` / `EXPIRED`（唯二真缺的；其余五种用项目自己的词），五个非可执行状态收进 `NOT_ACTIONABLE`，`GoalState.priority` 只读它一次；② 相位如实标注（FINISHED→EXPIRED、SCHEDULED/PREPARING→SCHEDULED_NOT_OPEN、DISCOVERED→UNKNOWN），客户端已过期的 `EVENT_MINIMUM_GUARANTEE` 从"负 deadline 的 READY"改 `EXPIRED`（`deadline_pressure(0)`=−100000 只会压低、永不挪出排序）；③ `event_goal.known_activities()` 读**既有**活动注册表（只认它自己 policy 认可的 gate）+ `_append_known_activities` 为已知活动留**计划票**（`available_skills=()`、状态取窗口、evidence 带 prepare/knowledge/occurrence、已被本帧读到的活动不重复发）；④ 等待改 `BLOCKED` + `retry_after`（队列/兵营自己的倒计时）+ `evidence["condition"]`，`completion`/`distance` 一字未动 ⇒ 排程不变、记录变真。**没有第二套任务系统。** **Q2**：注册表按 §二 补齐（适用角色/周期/下一次开放条件/参与条件/知识/历史成功路径/准备事项/本次完成与领取状态）；`BEAR_HUNT` REVIEWED 可计划、`STATE_VS_STATE` DISCOVERED 按政策不进计划；**没有任何活动有可靠 `start`/`end`**（从未量到）⇒ 按 §二**保留 null 不编造**，窗口由客户端印的东西回答。**真机**：`learning/goal_state.json` 13:57:21Z（站在 `MAIL` 页）已有 **`SCHEDULED_BEAR_HUNT EXPIRED priority=None window=EXPIRED prepare=3`** ⇒ 不在那一屏计划也在、不可调度所以不会去找不存在的入口。 **Q4**：改前 `OPEN_MAIL` 97（`NO_READING` 83/`PRESENT` 14）、`OPEN_ALLIANCE_GIFTS` 97（瓦片 `ABSENT` 93），两者合计 **176 次"无读数仍进页"**；**改后 29 步：各 0 次**；同期 `MAIL_CLAIM_REWARDS` 13:56:13 **成功**并继续 `SELECT_MAIL_ALLIANCE_TAB` ⇒ PRESENT 时照做。 **Q5**：改后 29 步 / 2 轮，继续做 `AVOID_STAMINA_WASTE` 9、`MAIL_ROUTINE` 9、`CLAIM_EXPLORATION_IDLE` 6、训练 3、科研 1 —— **不是全局 SAFE_STOP**（不可执行项 `priority=−inf`，连消耗一个周期的机会都没有）。 **Q3 答不出来且不编**：本项目**从未在活动窗口内出现过**（活动实例步数 0），判据改为留档：窗口推进由客户端自己的倒计时驱动（`bear_phase` 600s→PREPARING、120s→READY），来自运行正站着的那一帧。 **回归**：24 文件批 **439 passed / 7 failed**，7 条在"回退本轮""回退两轮"三种模式下**逐条同名** ⇒ **零回归**；本轮 8 个目标层文件 + 新验收文件 **180 + 22 passed / 0 failed**，回退本轮后分别有 **7 / 10 条重新变红** ⇒ 新测试真的在测新规则（并因此修好 **5 条既有红项**：`DeferredGoalScheduling` ×3、`EpisodeMeasurement` ×2，根因是它们断言的是"哪个巡检票赢五路同价"，而那个平局由机器上学到的 `history_bonus`/公平账本分开——把夹具从这两个机器状态里摘出来即稳定）。`check_wiring` **9 项与基线逐条相同**（含一条既存的 `goals: …no route for [...]`，那 8 个目标是刻意走无目标巡检的）。证据 `dataset/truth_audit/activity_windows_20260923/`（README + activity_windows.json）。 ⚠ **未做**：① **五路同价**（裸 MAP 帧上五个未读巡检票都 180）本身是设计弱点，已登记未改；② `OPEN_POWER_DETAILS` 仍是 #113（点击对、后帧确为实力详情、页面识别读成 UNKNOWN），本轮未动；③ `STATE_VS_STATE` 无下次开放时间，只能等真机；④ §七 仅两项在真机窗口内有数（无红点 4/5/6），活动类的 1/2/3 需要一次真实活动窗口。 |
| 116 | **快捷面板已经把城内状态读进来了，目标层却一个字都看不到；于是绕去实力详情"问第二遍"，而那趟 20 步全部失败** | 🟠 **四处已改（状态入 WorldState / 行身份 / 死点控件让位 / 状态已答不再绕路）；离线与真机语料证据齐，`AB_QUICK_PANEL_STATE=off` 逐站点复现旧行为** | **指令**：面板优先提供城内状态与对应功能入口；当前页能做完的直接做完；面板不可用才走其他可靠路径；做完一项继续下一项。 **断点一（状态丢弃）**：`ocr.py::_with_quick_panel` 只并 `camps`。实测**面板展开的 57 帧里**：`panel.building` 有读数 **56**、`panel.research` **57**，而 `WorldState.building` / `WorldState.research` **全 0** ⇒ 读到了、丢了，目标层看不见。**断点二（绕路）**：`brain.py` 空闲兵营分支在"行不可点"时返回 `OPEN_POWER_OVERVIEW`（加成总览），理由却写 `quick_panel_<camp>_is_idle`。全史 `train_goal_power_overview` 10 步 + `research_goal_power_overview` 10 步 **全部 FAILURE（`POWER_DETAILS_NOT_PROVEN`）**；`OPEN_POWER_OVERVIEW` 131 + `OPEN_POWER_DETAILS` 139 = **270 步（占 7168 的 3.77%）**。run `20260923_215819_292688` 6 步里 **4 步**是绕路与善后，而面板 13:59:41 已读齐三兵营+科研。**断点三（行身份）**：`goal=LANCER_CAMP_TRAINING` 被回以 `quick_panel_shield_camp_is_idle`（**盾兵**行）——扫描取字典序第一个空闲兵营。 **改法四处**：① `_with_quick_panel` 并入 `building`/`research`，**页面读到的不被覆盖**（与 camps 同规则）；② 空闲兵营**先取目标自己那一个**；③ 新增 `_without_an_enter_control`（一条规则一个家）把三种情形分开：**没有该行 / 控件槽空（`NONE`）/ 已点过而页面没开（次数用尽）** → §一.6 旧路线兜底且理由写明是哪一种；**画了控件但是已量到的死点（`DONE` 绿勾）** → **非致命 SAFE_STOP 让位**，不再绕路；④ 新增 `_panel_answered_this_goals_state`，训练/科研的 HOME 兜底不再去开加成总览，旧路线只留给"面板没在给这个目标干活"，理由由 `*_goal_requires_power_route` 改为 `panel_did_not_serve_this_goal_so_the_power_route_is_the_fallback`。 **绿勾不是点它**（关键取舍）：三兵营读 `已完成` 时客户端在**按钮列同一小方框**里画绿勾（`done_box_norm` x=0.5333 w=0.0569 = 科技研究行扫到的 `arrow_box_norm` **同一格**），但**其中心已被点过**（404,556）：面板关闭、无奖励弹窗、17:53:39 重读**仍是 `已完成`/`DONE`** ⇒ `_QUICK_PANEL_ROW_CLAIM_SKILL` 保持空集，未把绿勾当控件。 **§八 逐条**：①训练状态改从面板读 **已改**；②**至少一个空闲兵营真正经面板行进入并开始训练——本轮之前就已成立**：`07:27:02 OPEN_TASK_FROM_QUICK_PANEL_LANCER`（`verifier_ok=True`，到达**城内兵营动作条**）→ `07:27:27 OPEN_INFANTRY_TRAINING` → `07:27:50 TRAIN_TROOPS` **SUCCESS on TRAINING** → `07:28:15 SELECT_TRAINING_CAMP`；③科研**部分成立**：10 次面板行进入到达**实验室动作条**（`13:50:17 → 13:50:33 OPEN_RESEARCH → RESEARCH`），"启动符合条件的研究"无成功证据；④**联盟捐献/免费招募/我的奖励 未打通**：三区块在 2026-09-22 22:02 探索里读到（`可捐献25/25`、`免费招募`），**当前语料各只有 1 帧、零次进入步**（技能表与行键都在，只是从未出现在可见窗口）；⑤三兵营均忙→无空闲→非致命让位 **已成立**；⑥**滚动查找未做**（见未做）；⑦**已在正确页不回主城 已成立**：真机训练页帧（`status=AVAILABLE`/`trainable`）→ `TRAIN_TROOPS`，盾兵目标在射手页 → `SELECT_TRAINING_CAMP`；⑧四个旧理由字符串全退役，换成说明"是哪种兜底"的理由。 **面板行进入的实际到达率**（35 步）：带验证器证据的 **14** 步中 **12 步到达该任务城内动作条**、2 步什么都没到；其余 **21** 步 `verifier_evidence` 为空 ⇒ **无法据此判断，不当作成功也不当作失败**（"35/35 没落地"是第一版探针的判据错误：兵营行的设计结果就是城内动作条，**不是**训练页——已在 `learning/_probe_panel_entries.py` 的 docstring 里写明这次更正）。 **回归**：目标层+面板 15 文件批见下；`AB_QUICK_PANEL_STATE=off`（逐站点复现旧行为，含两个旧理由字符串）下 **8 条重新变红**，正是本轮新写的那些 ⇒ 它们真的在测新规则。 ⚠ **未做**：① **§一.5/§八.6 面板内部滚动未做**——2026-09-22 22:02 的既有探索（`_panel_scroll.py`，未入版本库）在面板内滑 5 次读数**完全相同**，这既可能是"到底了"也可能是"滑动没生效"，**日志无法区分**，需要设备测量而 AUTO 正占着设备；`entry_badges.quick_panel_badges` 已把未读行记成 `UNKNOWN`+`row_not_read_this_frame`（**不是 ABSENT**，§二已满足）⇒ 滚动是**发现能力**缺口不是正确性缺口；② 本轮改后窗口只有数步，**不足以单独支撑"减少量"结论**，before 数字是主力；③ 绿勾行仍无人能收。 |
