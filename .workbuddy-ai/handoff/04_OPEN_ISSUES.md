...
| 98 | **快捷面板行箭头的落点被扫描带外的蓝色拉偏：`arrow_norm.x` 报 0.56-0.67，按钮在 0.53-0.59，点偏即落空/关面板** | 🔴 **未修（机制已复现，改动缺依据）** | 帧 `runtime_auto/20260923_032158_020595/…step_018_before_20260922T193104559465.png` 逐像素复现：按钮本体青色 `(76,198,244)` 在 **x 388-425**，红点徽标在 412-420，面板右边缘约 476-484；而 `QUICK_PANEL_ARROW_BAND_X_NORM=(0.45,0.80)` 的右沿是 **576**（面板外 155px 的城市），`_panel_arrow_and_badge` 取该带内蓝色的 **min..max** 且**取最宽的一行为准** ⇒ 选中被污染的行：`scan_y=634` 得 384-498（中心 0.6125）、`scan_y=694` 得 386-574（0.6667）。四条行读出的 `arrow_box_norm.w_norm` 各不相同（0.1292 / 0.1625 / **0.2611** / 0.1306），同一控件宽度本该一致 ⇒ 自相矛盾可检。 **后果（有账本对照）**：用 `arrow_norm` 的 tap 落在按钮外——`19:32:04` 科研行 tap `[448,804]` ⇒ 面板**没关也没动**、`PANEL_ROW_RESEARCH_BAR_NOT_PROVEN`；`17:16:14` `[480,804]`、`17:54:12` `[448,804]` 同样失败。而 `18:55:10` `[404,804]`（落在按钮 388-425 内）**SUCCESS**，`16:33:10`/`16:41:59` `[225,546]`（落在行标签上）也 **SUCCESS**。 **已试过但未采纳的两条修法**：① 合并间隔 ≤12px 的蓝段后取最宽组——在 19:32 那批**四行都恰好给出 0.5618（按钮中心）**，但在 19:36 那批反而更差（0.69-0.77）；② 用跨度做守卫——**缺依据**：本批按钮宽 41px，而 2026-09-22 上午那批按常量注释是 76-105px，宽度本身不一致。 **下一步**：先用一个**不依赖蓝色带**的定位器量出按钮宽度（候选：按钮内的白色雪佛龙，或面板卡片的几何），再据它设上下界；在此之前不要改取值规则，也不要给 tap 路径加阈值。 | 影响面：所有 `QUICK_PANEL_ROW_*` 的进入类落点（`_resolve_semantic_target`）。 |
