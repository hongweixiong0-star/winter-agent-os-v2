...
| 99 | **快捷面板行的 `label` 可以取自面板之外的文字；同一行因此被读成"非空闲"** | 🔴 **未修（有帧）** | 帧 `learning/_panel_scroll/02_after_scroll_1.png`：读取器报 `RESEARCH` 行 `label=梦境寻忆, status=IN_PROGRESS, source_word=None, control=NONE, y=0.2977`；而该帧**全部 OCR token** 里 `梦境寻忆` 只出现在 **x 0.769**（面板右缘约 0.585，那是右侧活动栏），面板内该行真正的文字是 `科技研究`（y 0.3211，x 0.311）与 `空闲中`（y 0.3445），并画着一颗蓝箭头。⇒ 两条后果：① 一行取到的标签可以**根本不在这一行上**；② 真正空闲的科研被读成 `IN_PROGRESS / control=NONE`，于是 `RESEARCH` 路由会**跳过它**（对照：同一槽位在真机 `20:03` 读作 `科技研究 / 空闲中 / control=ARROW / badge=PRESENT`）。机制尚未定位（该行的 `y` 比其他行相对其标签高约 0.023，怀疑首行锚点取自分区标题而非行标签）；**下一步**：先量 y 锚点与标签/状态词的相对偏移在多少帧上成立，再决定是修锚点还是要求"标签必须落在本行 x 带内"（后者与现有 `SECTION_ROW_KEYS` 的按节命名并不冲突）。 | 影响面：`QUICK_PANEL_ROW_*` 的 `label`、以及任何按 `status` 判"是否空闲"的路由（`PANEL_ROWS_FOR_ROUTE`）。 |
