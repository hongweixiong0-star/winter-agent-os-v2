...
| 97 | **已完成后客户端画的是第三种营地图：金环光晕 + 通用提示气泡，项目一个判据都读不到** | 🟠 **已定性（有帧）** | 帧 `dataset/raw/live_runtime/live_runtime_step_008_after_refresh_2_20260922T185419887295.png`：快捷面板盾兵行 `status=IDLE / control=DONE`（绿勾）被按之后，客户端画出的是**营房被金环光晕选中 + 上方一只白色提示气泡（里面是火车/队列图标）**，**没有**径向菜单、**没有**地面金椭圆；读取器给出 `training={}`，于是该步记 FAILURE 且路由无点可打。同一渲染在 `17:54:28`（按行体后）也出现。**为什么读不到**：`camp_ring.py` 的环检测要求金色 bbox ≥ 0.12×0.05（地面椭圆才行，光晕太小）；`_read_selected_building` 要求工具条的 `训练` 词 + 军营名，而这一渲染根本不画工具条。**下一步（先量再做）**：给这一渲染补一个判据——它的气泡内容是不透明图标（营地为士兵头像/火车图标），适合做模板；**不得**再用「有白气泡」当判据（见下条实测）。 | 关联：B 项（真正领取已完成批次）卡在这里 —— 已是第 3 种渲染，前两种（地面金环 `camp_ring.py`、工具栏 `_read_selected_building`）都各有读法，这一种没有。 |
