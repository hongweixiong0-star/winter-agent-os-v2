...
| 97 | **被按下的"已完成"行的结局读不出来（成因未定）** | 🟠 **已定性 + 因果已撤回** | 事实部分：`18:54:27`（面板盾兵行 `status=IDLE / control=DONE` 被按）之后，帧 `dataset/raw/live_runtime/live_runtime_step_008_after_refresh_2_20260922T185419887295.png` 上 `training={}`，路由无点可打。**因果撤回**：我原先写"这次 tap 画出了金环光晕+气泡"，不成立——① 用项目自己的模板判定，气泡下方那座建筑是 **`BUILDING_RESEARCH_CENTER` 0.654**（兵营只有 0.257），是**科研所**不是兵营；② 同一标记在同一技能的**无关步骤**帧上也出现（`17:55:01` 得 0.655）。⇒ 与 #92 同类：**客户端自己画的标记不能归因于我们的动作**。另据 `19:31:04` 的面板读数，`盾兵` 行已回到 **`空闲中`**（不再是 `已完成`）⇒ 该状态是**瞬时通知**。 | **仍未定**：按下去到底发生了什么（面板关闭 + `training={}`）。下一步需要在真机上做一次**受控**实验：同一步取前后帧、并同时保存面板读数与页面读数。 |
| 98 | **快捷面板行箭头的落点被扫描带外的蓝色拉偏** | 🟢 **已修（ocr + 测试）** | **成因（逐像素复现）**：扫描带 `(0.45, 0.80)` 右沿 = x 576，而面板右缘约 476-484 ⇒ 带内含 155px 的蓝色城市；蓝的**范围取 min..max**、且**取最宽的一行为准**，于是**优先**选中被污染的行。`19:31:04` 帧四行读作 384-477 / 384-498 / 386-574 / 401-494（中心 0.5979-0.6667，最后一个已在面板外），而按钮本体青色 `(76,198,244)` 在 **x 384-425**。**账本证据**：用 `arrow_norm` 的三次 tap 全失败（`19:32:04 [448,804]` 面板没关没动 + `PANEL_ROW_RESEARCH_BAR_NOT_PROVEN`、`17:16:14 [480,804]`、`17:54:12 [448,804]`），而落在按钮内的 `18:55:10 [404,804]` 与落在行标签的 `16:33:10/16:41:59 [225,546]` 都成功。 **修法**：改用**按钮内的白色雪佛龙**定位按钮（`QUICK_PANEL_CHEVRON_MIN=245`，因面板压暗城市、最亮城市像素只有 200-227），再把蓝色只在该邻域内读（`QUICK_PANEL_BUTTON_PAD_PX=12`）；徽标窗口从"按钮左三分之一"改为**右三分之一**（红点实测在 412-420，即按钮右端；旧注释与旧窗口都是照着被污染成 0.539-0.644 的框写的，已一并更正）。 **定位器实测**（10 帧面板 × 4 行 = 40 次）：**37 次命中，且每次都是 x 396-416（宽 20px，中心 0.5639，spread 0.0000）**；3 次未命中**恰是 已完成 行**（19:03:02/19:03:12/19:10:52 的盾兵行——那里画的是绿勾不是箭头）⇒ "没有雪佛龙"就是"客户端没画进入箭头"的正确答案。 **验证**（8 帧面板重读）：`arrow_x` **全部回到 0.5618**（按钮中心，原 0.5979-0.666），徽标状态**一条未变**；顺带的效果是 已完成 行现在落到带标签的估算回退（进入路径本就拒绝它）⇒ "已完成行不是进入目标"由**构造**保证，而不是靠手写条件。 **未在真机观察**：修好后尚未有真机轮再走这条面板行路径。 | 影响面：`QUICK_PANEL_ROW_*` 的进入类落点。原 `QUICK_PANEL_ARROW_BAND_X_NORM` 与死变量 `gap_px` 已删除。 |
