...
> **后续（同日更晚）**：产品定义落地与身份来源见
> `docs/PRODUCT_ONE_AGENT_MULTI_ROLE.md`。两点补充证据：
> 1. **多角色已被证实**（不是假设）：语料跨了两个账号 —— 09-14 帧战力 70,206,322 / 面板
>    `行军 6/6`，09-16 帧战力 542,443 / 面板 `1/2`，同服务器 `#4298`。本文件 Q6/Q7 说
>    "无法观测这是哪个角色"依然成立，但**身份来源已经找到**：`领主档案` 面板（点一次头像），
>    见 `dataset/truth_audit/role_identity_20260916/`。
> 2. **Q1 的容量数字还有一个解析缺陷**（本轮已修）：`read_march_count` 原来把 ROI 的多个
>    token 拼成一行再匹配，于是 `'3/' ＋ '3/6'` 被读成 `3/3` —— 客户端说 6，episode 记 3。
