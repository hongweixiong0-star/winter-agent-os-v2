...
## 4. 边界要能被"移动"，否则 A/B 是假的（本轮踩过）

把 `MAX_UNRESOLVED_ATTEMPTS` 写在 `run()` 里（跟旁边 `MAX_ORDINARY_ATTEMPTS` 一样的写法）后，
A/B 插件改**类属性**被 per-run 赋值覆盖 ⇒ **两腿结果完全相同**，看起来像"零回归"，
其实那次对照没有检验任何东西。**改了边界就要验证边界真的动了**：

- 能写成**类常量**的边界就写成类常量；per-run 赋值只在该值每轮真的不同时才用。
- 测试里同时跑两腿（本项目：`bound=2` 与 `bound=10**9`），让**负向对照**（无界 → 更多次尝试）
  证明这个界是它起作用，而不是别处顺手修好了。
- 判断标准：如果关掉/放开这个界之后**行为没有变化**，那这次 A/B 什么也没证明 —— 先查"界有没有真的被读到"。

同类陷阱：`_InertXxx` 之类的"惰性映射"替换实例字段时，若代码在该字段上另有写入路径，惰性会失效。
更稳的做法是改**读到的那个值**（常量/阈值），而不是改容器。

### 夹具自己的输入也是变量——把它们也钉住

断言"这一步应该是什么"的整轮测试，会**同时**依赖机器上的学习产物。本项目 2026-09-23 量到：
裸 MAP 帧上五个未读巡检票**同价 180**，谁赢由 `goal_utility.load_routes()`（能力自举会重写的
`knowledge/strategy/stamina_routes.json`）给出的 `history_bonus` 与公平账本
（`learning/goal_fairness.json`，运行中的 AUTO 一直在写）分开。于是那些测试断言的是**这台机器当前状态**，
不是代码——该项目自己的 docstring 已经承认它们是"the machine's own ledgers 的受害者"。

做法：一个 `@contextmanager offline_runtime_state(temp, observations=None)`，把两处指向受控文件，
并允许夹具**说出它已知什么**（`observations` 写成一条真实读数，让"已读且无事"的巡检票退出平局）。
本项目因此从 21 条红项降到 2 条、并顺手修好 5 条既存红项。

**反面教训（同一件事的另一半）**：不要为了"让夹具确定"去摘掉生产项。摘掉 `ROUTES_PATH` 之后
`history_bonus` 一没、平局退化为板序，而守着"deferred 不该挤掉仍可选的活"的那条测试**立刻报出它守的 bug**
——说明那一项是承重的生产行为，删掉它等于"把测量对象改坏来修测量"。发现这一点就回退，并把原因写在原地。

### A/B 的模式要能**分开两轮改动**，不能只有一个开关

同一批测试里往往叠着好几轮改动。本轮里"只回退新一轮""只回退上一轮""两轮都回退"三种模式给出的失败集合
**互不包含**（7 / 14 / 5），任何两个模式的差集都会把别人那轮的失败算到自己头上——
只有四格齐全才能算清"哪条是谁引入的"。另加一条：`AB_*` 开关必须是**逐站点复现旧语义**，
"一律 PRESENT"这种近似会同时改掉一条本来就正确的行为，基线就不是基线。
