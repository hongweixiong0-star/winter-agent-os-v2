...
定向验证会撞到"这个视图里没有已登记的退出控件"（本轮：世界地图的**资源搜索视图**不画城镇门，
`BTN_OPEN_HOME` 不在帧上，探针按设计中止而**不猜**）。遇到时不要给探针加一条临时出路：
先把同一状态在 `episodes.jsonl` 里查一遍——本轮查出来 `OPEN_HOME` **44/323 次失败**、
全部 `page_before=MAP`，其中 22/30（抽样）就是"搜索面板盖住门"，还有 4 次"控件在帧上却解析不出落点"、
4 次"tap 发出后客户端答以弹窗"。**探针的中止点常常正是运行时的失败点**，把它登记成 issue，
比给探针抄一条捷径有价值。
