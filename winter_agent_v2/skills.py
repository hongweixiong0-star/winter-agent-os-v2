...
def v2_registry() -> SkillRegistry:
    ...
    skills.extend([
        ...
        Skill("SEND_MARCH", "Send a parameterized march for gather, attack, reinforce, scout, or occupy", None,
              ...
              ui_change_tolerance=("icon", "number", "position", "resolution", "skin")),
        Skill("RECALL_MARCH", "Recall one selected march to release a normal slot", None,
              Action("RECALL_MARCH", payload={"march_id":"FROM_GOAL", "reason":"FROM_GOAL"}),
              state=SkillState.CANDIDATE, latency_class=LatencyClass.FAST,
              verifier="NORMAL_IDLE_SLOT_INCREASED", recovery=("REFRESH_MARCH_STATE",),
              semantic_goal="Release a normal march slot", parameters=("march_id", "reason"), context=("WORLD",),
              semantic_requirements=("march_identity_known", "recall_action_available"),
              vision_evidence=("march_state", "recall_action_state"), unknown_policy="Unknown march identity blocks",
