...
## 八、⚠ 真机验证失败：点环心也开不了菜单（2026-09-20T18:43:51Z）

修复上线后 **AUTO 真机跑了 1 次**这一跳，**结果失败**：

```
repo_rev    : 2f616406d4bc87343fcbb42a72ef6aae378dd02d   ← 含本次修复
goal_id     : KEEP_TRAINING_PRODUCTIVE
skill       : SELECT_INFANTRY_CAMP
action      : TAP_SEMANTIC: TRAINING_CAMP_IN_RING
before.training.camp_tap_norm = [0.441, 0.4551]          ← (317, 583) = 环的中心
action_backend = ADB                                     ← 点击真的发出去了
result      : FAILURE / INFANTRY_CAMP_MENU_NOT_PROVEN
after.page  : HOME      after.training : {}              ← 既无菜单，也无高亮
```

**after 帧（`key/09_*`）证实的**：径向菜单**没有出现**，而**金色高亮环消失了**
（对比 `key/08_*` 的 before 帧，环还在）——**环消失 = 客户端把这次点击当成了"点空"**。

**标注图（`key/10_*`）**：红色十字是本次实际点击的 (317,583)，**它确实落在环内**（压在屋顶上）。
而**教学手指的指尖指向的是环右侧那个带 `2` 角标的图标**（测得 (379,592)，98 px、12×15 的紧凑亮块）。

**环内有 5 个亮色块**：(334,547) 264px 细长、(379,592) 98px 方正、(316,537) 60px、
**(301,584) 60px**（我点的 (317,583) 紧邻它）、(302,561) 44px。
⇒ 手指指向的那块可以被测出来，**但"5 个候选里选哪一个"没有客观唯一判据**，所以没有据此再改落点。

⇒ **stage A 现在四次真机点击、四次失败**：

| 次 | 落点 | 结果 |
|---|---|---|
| 1–3 | (346, 682) 环外空地 | 菜单没开 ×2；**跳世界地图** ×1 |
| **4** | **(317, 583) 环内** | **菜单没开，环消失** |

**"瞄得更准"不是答案。** 因此 `brain.py` **撤回该点击**，回到"等待"（数值上与 09-17 之后一致），
并保留 `camp_ring.py` 与 `training["camp_tap_norm"]`——**环的测量本身是可靠的**
（46/46 帧、sd 4 px），它只是**指向了一个点不动的目标**。

## 九、下一步不是调坐标，是查清"这个状态是什么"（#82）

**新假设（待验证，不是结论）**：那不是一个**圆形**的建筑选中框，而是一个**椭圆**
（rx=102, ry=56），画在一个**大型多部件建筑**上，**带 `2` 角标**，且**有教学手指指着那个角标**
—— 这正是**引导式步骤（guided step）**的样子，而不是"兵营被选中"。

**若该假设成立**，则 `verify_infantry_camp_highlighted` 接受这一帧就是**假到达**，
训练路线的前半段一直在报一个**它从未获得过的成功**。

**可判定的下一步**（都不需要盲点坐标）：
1. 在 stage A 帧上 OCR **环内/附近的建筑名**（若客户端显示），与 `knowledge/game/buildings.json` 比对；
2. 查是否有**教程/引导态**的视觉证据（项目目前**没有任何 tutorial 语义**，见 04_OPEN_ISSUES #74 记录）；
3. 若确为引导态，则应记录为**前置条件未满足**（`DEFER`），而不是继续寻找点击点。
